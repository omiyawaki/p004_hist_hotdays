...
def plot(md):
    odir = '/project/amp/miyawaki/plots/p004/cesm2-le/%s/%s/%s/%s' % (se,fo,md,varn)
    ...
    # ax.contourf(mlon, mlat, sig[str(p)], 3, hatches=['','....'], alpha=0, transform=ccrs.PlateCarree())
    ax.coastlines()
    if md=='mmm':
        ax.set_title(r'%s LENS2 Mean (%s)' % (tavg.upper(),yr))
    else:
...
# [plot('CESM2.%s'%i) for i in tqdm(lens)]
