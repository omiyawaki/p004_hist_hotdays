...
for reg in lreg:
    for se in lse:
        ...
        idir2 = '/project/amp/miyawaki/data/p004/hist_hotdays/era5/%s/%s' % (se,varn2)
        odir = '/project/amp/miyawaki/plots/p004/hist_hotdays/era5/%s/%s+%s' % (se,varn1,varn2)
        ...
        for ipc in range(len(lpc)):
            ...
            for ihr in range(7):
                ...
                plt.legend()
                plt.tight_layout()
                plt.savefig('%s/scatter.%s.%s.%02d.%s.%s.%02d.pdf' % (odir,varn1,varn2,pc,se,reg,ihr), format='pdf', dpi=300)
                plt.close()
