...
def plot(re):
    ...
    def ebinned(vn,w,idx,nb):
        ...
        return bvn
    ahddpe=ebinned(ahddpe,ahw,iai,nb)
    ...
    ahddp1=binned(ahddp1,ahw,iai,nb)
    ...
    cat=xr.open_dataarray('%s/ai.%s.%s_%s.%s.nc'%(ddir,varnc,his,fut,re))
    ...
    # ax.set_xticklabels(np.arange(2,12+2,2))
    # ax.set_xlabel('Month sorted by increasing AI')
    if ylb:
    ...
    fig.savefig('%s/ai.ddp%02d%s.%s%s.ah.ai.sign.%s.png' % (odir1,p,varn1,fo,fstr,re), format='png', dpi=600,backend='pgf')
    fig.savefig('%s/ai.ddp%02d%s.%s%s.ah.ai.sign.%s.pdf' % (odir1,p,varn1,fo,fstr,re), format='pdf', dpi=600,backend='pgf')
