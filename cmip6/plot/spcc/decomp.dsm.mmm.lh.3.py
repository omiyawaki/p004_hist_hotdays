...
def plot():
    ...
    ld=[ooplh_fixbc,ooplh_fixmsm,ooplh_rddsm] # store in list
    ls=[vstr('ooplh_fixbc'),vstr('ooplh_fixmsm'),vstr('ooplh_rddsm')]
    ...
    # for m in tqdm(range(12)):
        ...
        fig,ax=plt.subplots(nrows=1,ncols=3,subplot_kw={'projection': ccrs.Robinson(central_longitude=240)},figsize=(9,3),constrained_layout=True)
        ax=ax.flatten()
        if m==0:
        ...
        for v in range(3):
            if m==6 and v==2:
                clf=ax[v].contourf(mlon, mlat, ld[v][m,...], np.arange(-vm,vm+dvm,dvm+dvm*1e-1),extend='both', vmax=vm, vmin=-vm, transform=ccrs.PlateCarree(), cmap='RdBu_r')
            else:
