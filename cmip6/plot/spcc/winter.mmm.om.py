...
def plot(vn):
    ...
    if tr:
        ...
        fig.savefig('%s/djf+jja.ddp%02d%s.%s.%s.tr.om.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
    ...
    if nhhl:
        ...
        cb.set_label(label=r'$\Delta \delta %s$ (%s)'%(varnlb(vn),unitlb(vn)),size=16)
        fig.savefig('%s/djf+jja.ddp%02d%s.%s.%s.nhhl.om.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
    ...
    cb.ax.tick_params(labelsize=16)
    cb.set_label(label=r'$\Delta \delta %s$ (%s)'%(varnlb(vn),unitlb(vn)),size=16)
    fig.savefig('%s/djf+jja.ddp%02d%s.%s.%s.om.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
