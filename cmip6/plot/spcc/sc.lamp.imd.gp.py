...
def plot(md,re):
    ...
    ax.contour(xx,yy,np.where(zz<cval,0,1),levels=[0.5],colors='k',linewidths=0.5,corner_mask=False)
    if showcb:
...
def wre(md):
    [plot(md,re) for re in lre]

[wre(md) for md in tqdm(lmd)]
