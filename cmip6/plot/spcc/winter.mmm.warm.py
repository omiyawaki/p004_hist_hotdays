...
# vnp='ooplh'
se = 'winter' # season (ann, djf, mam, jja, son)
...
def plot(vn):
    ...
    mvn=xsp.exp_taper(mvn,ntrunc=ntrunc)
    mvn=mvn.data
    ...
    if nhhl==True:
        ...
        fig.savefig('%s/winter.m.%s.%s.%s.nhhl.png' % (odir,vn,fo,fut), format='png', dpi=dpi)
    ...
    cb.ax.tick_params(labelsize=16)
    cb.set_label(label=r'$\Delta \overline{%s}$ (%s)'%(vnlb,unlb),size=16)
    fig.savefig('%s/winter.m.%s.%s.%s.png' % (odir,vn,fo,fut), format='png', dpi=dpi)
