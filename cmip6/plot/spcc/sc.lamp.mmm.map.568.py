...
lrseasons=cmr.get_sub_cmap('cmr.seasons',0.1,0.8)
...
lc=['salmon','salmon','yellowgreen']
fig,ax=plt.subplots(subplot_kw={'projection':ccrs.Robinson(central_longitude=240)},figsize=(4,1),constrained_layout=True)
for ie,i in enumerate([4,5,7]):
    clf=ax.pcolormesh(mlon,mlat,np.ma.masked_not_equal(ammsws,i),vmin=-1,vmax=0,transform=ccrs.PlateCarree(),cmap=mcolors.ListedColormap([lc[ie]]))
    if i==5:
        clf=ax.pcolor(mlon,mlat,np.ma.masked_not_equal(ammsws,i),vmin=-1,vmax=0,transform=ccrs.PlateCarree(),cmap=mcolors.ListedColormap(['none']),hatch='++++',lw=0,edgecolor='yellowgreen')
...
gl.top_labels=False
fig.savefig('%s/maxmon.ddp%02d%s.%s.msws.568.png' % (odir1,p,varn1,fo), format='png', dpi=600)
# fig.savefig('%s/maxmon.ddp%02d%s.%s.msws.568.pdf' % (odir1,p,varn1,fo), format='pdf', dpi=600)
