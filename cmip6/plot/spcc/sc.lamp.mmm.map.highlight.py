...
for i in range(12):
    ...
    clf=ax.pcolormesh(mlon,mlat,np.ma.masked_not_equal(ammsws,i),vmin=-1,vmax=0,transform=ccrs.PlateCarree(),cmap='Blues')
    ax.coastlines()
    ...
    gl.top_labels=False
    fig.savefig('%s/maxmon.ddp%02d%s.%s.msws.%02d.png' % (odir1,p,varn1,fo,i+1), format='png', dpi=600)
    # fig.savefig('%s/maxmon.ddp%02d%s.%s.msws.%02d.pdf' % (odir1,p,varn1,fo,i+1), format='pdf', dpi=600)
