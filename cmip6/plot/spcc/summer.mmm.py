...
def plot(vn):
    ...
    if nhhl:
        ...
        fig.savefig('%s/jja+djf.ddp%02d%s.%s.%s.hl.pdf' % (odir,p,vn,fo,fut), format='pdf', dpi=dpi)
        fig.savefig('%s/jja+djf.ddp%02d%s.%s.%s.hl.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)

    if af:
        # plot Africa ONLY
        fig,ax=plt.subplots(subplot_kw={'projection':ccrs.Robinson()}, figsize=(4,6),constrained_layout=True)
        # ax.set_title(r'%s %s' % (md.upper(),fo.upper()),fontsize=16)
        ax.set_title(r'%s %s JJA+DJF' % (md.upper(),fo.upper()),fontsize=16)
        clf=ax.contourf(mlon, mlat, llddpvn, np.arange(-vmdd,vmdd+dvmdd,dvmdd),extend='both', vmax=vmdd, vmin=-vmdd, transform=ccrs.PlateCarree(),cmap='RdBu_r')
        ax.coastlines()
        ax.set_extent((-25,60,-40,0),crs=ccrs.PlateCarree())
        # gl=ax.gridlines(crs=ccrs.PlateCarree(),draw_labels=True,linewidth=0.5,color='gray',y_inline=False)
        # gl.ylocator=mticker.FixedLocator([-50,-30,0,30,50])
        # gl.yformatter=LatitudeFormatter()
        # gl.xlines=False
        ...
        cb.ax.tick_params(labelsize=12)
        cb.set_label(label=r'$\Delta \delta %s$ (%s)'%(varnlb(vn),unitlb(vn)),size=16)
        fig.savefig('%s/summer.ddp%02d%s.%s.%s.af.pdf' % (odir,p,vn,fo,fut), format='pdf', dpi=dpi)
        fig.savefig('%s/summer.ddp%02d%s.%s.%s.af.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
    ...
    cb.set_label(label=r'$\Delta \delta %s$ (%s)'%(varnlb(vn),unitlb(vn)),size=16)
    fig.savefig('%s/summer.ddp%02d%s.%s.%s.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
