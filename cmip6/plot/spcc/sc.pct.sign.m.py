...
def plot(re):
    ...
    fig.savefig('%s/sc.%s.%s%s.ah.pct.sign.%s.m.png' % (odir1,varn1,fo,fstr,re), format='png', dpi=600,backend='pgf')
    fig.savefig('%s/sc.%s.%s%s.ah.pct.sign.%s.m.pdf' % (odir1,varn1,fo,fstr,re), format='pdf', dpi=600,backend='pgf')
