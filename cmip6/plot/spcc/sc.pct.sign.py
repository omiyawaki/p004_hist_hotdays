...
def plot(re):
    ...
    dpvn1=load_mmm(varn1,varnp)
    pct=dpvn1['percentile']
    if reverse and (varn1 in ['ti_ev','gflx','fsm','hfss','hfls','fa850','fat850','advt850','advtx850','advty850','advm850','advmx850','advmy850','rfa'] or 'ooplh' in varn1):
        dpvn1=-dpvn1
    if 'wap' in varn1:
        dpvn1=dpvn1*86400/100 # convert from Pa/s to hPa/d
    ...
    def load_vn(idir0):
        dpvne=xr.open_dataarray('%s/dpc.%s_%s_%s.%s.nc' % (idir0,varn1,his,fut,se))
        if varn1=='pr': dpvne=86400*dpvne
        return dpvne
    ...
    dpvne=[load_vn(idir0) for idir0 in tqdm(idirs)]
    dpvne=xr.concat(dpvne,'model')

    # remap to lat x lon
    tvn1=remap(tvn1,gr)
    dpvn1=remap(dpvn1,gr)
    dpvne=eremap(dpvne,gr)
    ...
    nidx=np.isnan(dpvn1)
    for imd in range(dpvne.shape[0]):
        nidx=np.logical_or(np.isnan(nidx),np.isnan(dpvne[imd,...]))
    tvn1[nidx]=np.nan
    dpvn1[nidx]=np.nan
    dpvne[:,nidx]=np.nan
    ...
    # select region
    ahdp1,kidx=regsla(dpvn1,gr,ah)
    tvn1,_=regsla(tvn1,gr,ah)
    ahdpe=eregsla(dpvne,gr,ah,kidx)
    # weights
    ahw=regsl2d(awgt,ah,kidx)

    # area weighted mean
    ahdpg=ahdp1.copy()
    ahdp1=np.sum(ahw*ahdp1,axis=-1)/np.sum(ahw)
    ahdpe=np.sum(ahw*ahdpe,axis=-1)/np.sum(ahw)
    ...
    if bstype=='bins':
        ...
        if md=='mmm':
            # bootstrap
            s=ahdpe.shape
            ahdpe=np.reshape(ahdpe,(s[0],s[1]*s[2]))
            al=np.ones(ahdpe.shape[1])
            for ig in tqdm(range(ahdpe.shape[1])):
                sa=ahdpe[:,ig]
                bs=[np.nanmean(resample(sa)) for _ in range(nbs)]
                bs=np.array(bs)
                pl,pu=np.percentile(bs,100*np.array([alc/2,1-alc/2]))
    ...
    # ann mean
    aahdp1=np.nanmean(ahdp1,axis=0)
    fig,ax=plt.subplots(figsize=(4,3),constrained_layout=True)
    ax.plot(pct,aahdp1,'-k')
    ...
    clf=ax.pcolormesh(ahmmon,ahmpct,ahdp1,vmin=-vmax(varn1),vmax=vmax(varn1),cmap=cmap(varn1))
    if bstype=='bins' or md=='mmm':
        hatch=plt.fill_between([-0.5,11.5],0,100,hatch='///////',color='none',edgecolor='gray',linewidths=0.3)
        ax.pcolormesh(ahmmon,ahmpct,np.ma.masked_where(al==0,ahdp1),vmin=-vmax(varn1),vmax=vmax(varn1),cmap=cmap(varn1))
    ...
    fig.savefig('%s/sc.%s.%s%s.ah.pct.sign.%s.png' % (odir1,varn1,fo,fstr,re), format='png', dpi=600,backend='pgf')
    fig.savefig('%s/sc.%s.%s%s.ah.pct.sign.%s.pdf' % (odir1,varn1,fo,fstr,re), format='pdf', dpi=600,backend='pgf')
