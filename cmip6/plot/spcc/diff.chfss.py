        ...
        for fo in lfo:
            ...
            for md in lmd:
                ...
                if md=='mmm':
                    ...
                    vlim=20
                    clf=ax.contourf(mlon, mlat, stdev, np.arange(0,vlim+2,2),extend='both', vmax=vlim, vmin=0, transform=ccrs.PlateCarree())
                    ax.coastlines()
                    ax.set_title(r'%s %s %s' % (se.upper(),md.upper(),fo.upper()))
                    cb=plt.colorbar(clf,location='bottom')
                    if pc==0:
                        cb.set_label(r'$\sigma(\Delta \overline{SH}_\mathrm{2\,m})$ (W m$^{-2}$)')
                    else:
                        cb.set_label(r'$\sigma(\Delta SH^{>%s}_\mathrm{2\,m})$ (W m$^{-2}$)' % (pc))
                ...
                # transparent colormap
                vlim=20
                clf=ax.contourf(mlon, mlat, diff, np.arange(-vlim,vlim+2,2),extend='both', vmax=vlim, vmin=-vlim, transform=ccrs.PlateCarree(), cmap='RdBu_r')
                ax.coastlines()
                ax.set_title(r'%s %s %s' % (se.upper(),md.upper(),fo.upper()))
                cb=plt.colorbar(clf,location='bottom')
                if pc==0:
                    cb.set_label(r'$\Delta \overline{SH}_\mathrm{2\,m}$ (W m$^{-2}$)')
                else:
                    cb.set_label(r'$\Delta SH^{>%s}_\mathrm{2\,m}$ (W m$^{-2}$)' % (pc))
