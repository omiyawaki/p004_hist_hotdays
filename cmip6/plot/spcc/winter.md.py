...
def plot(md):
    ...
    if reverse and (vn in ['gflx','fsm','hfss','hfls','fa850','fat850','rfa'] or 'ooplh' in vn):
        dpvn=-dpvn
        ddpvn=-ddpvn

    # jja and djf means
    mvn1j =np.nanmean(dvn.data[5:8,:]  ,axis=0)
    dvnj  =np.nanmean(dvn.data[5:8,:]  ,axis=0)
    dpvnj =np.nanmean(dpvn.data[5:8,:] ,axis=0)
    ddpvnj=np.nanmean(ddpvn.data[5:8,:],axis=0)
    mvn1d  =np.nanmean(np.roll(dvn.data  ,1,axis=0)[:3,:],axis=0)
    ...
    # remap to lat x lon
    llmvn1j=np.nan*np.ones([gr['lat'].size*gr['lon'].size])
    ...
    llmvn1j[lmi]=mvn1j.data
    lldvnj[lmi]=dvnj.data
    lldpvnj[lmi]=dpvnj.data
    llddpvnj[lmi]=ddpvnj.data
    llmvn1j=np.reshape(llmvn1j,(gr['lat'].size,gr['lon'].size))
    ...
    llmvn1d=np.nan*np.ones([gr['lat'].size*gr['lon'].size])
    ...
    llmvn1d[lmi]=mvn1d.data
    lldvnd[lmi]=dvnd.data
    lldpvnd[lmi]=dpvnd.data
    llddpvnd[lmi]=ddpvnd.data
    llmvn1d=np.reshape(llmvn1d,(gr['lat'].size,gr['lon'].size))
    ...
    llmvn1j = np.append(llmvn1j, llmvn1j[...,0][...,None],axis=1)
    ...
    llmvn1d = np.append(llmvn1d, llmvn1d[...,0][...,None],axis=1)
    ...
    if clima:
        ...
        clf=ax.contourf(mlon, mlat, llmvn1, np.arange(-vm1,vm1+dvm1,dvm1),extend='both', vmax=vm1, vmin=-vm1, transform=ccrs.PlateCarree(), cmap='RdBu_r')
        ax.coastlines()
        ax.set_title(r'%s %s DJF+JJA' % (md.upper(),fo.upper()),fontsize=16)
        cb=fig.colorbar(clf,location='bottom',aspect=50)
        cb.ax.tick_params(labelsize=16)
        cb.set_label(label=r'$%s^{%02d}$ (%s)'%(varnlb(vn),p,unitlb(vn)),size=16)
        fig.savefig('%s/winter.%s.%s.png' % (odir,vn,fo), format='png', dpi=dpi)
    ...
    fig.savefig('%s/winter.ddp%02d%s.%s.png' % (odir,p,vn,fo), format='png', dpi=dpi)
    ...
    fig.savefig('%s/winter.dp%02d%s.%s.png' % (odir,p,vn,fo), format='png', dpi=dpi)
    ...
    fig.savefig('%s/winter.d%s.%s.png' % (odir,vn,fo), format='png', dpi=dpi)
