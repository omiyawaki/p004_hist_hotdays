...
def plot(vn):
    ...
    if nhhl:
        if domp:
            ...
            ax.contour(mlon,mlat,mtas1,[0],transform=ccrs.PlateCarree())
            ax.contour(mlon,mlat,mtas2,[0],transform=ccrs.PlateCarree())
            ax.coastlines()
            ...
            fig.savefig('%s/djf+jja.d%02d%s.%s.%s.hl.frz.png' % (odir,50,vn,fo,fut), format='png', dpi=dpi)
            ...
            ax.contour(mlon,mlat,ptas1,[5],linestyles='dashed',colors='yellow',transform=ccrs.PlateCarree())
            ax.contour(mlon,mlat,ptas2,[5], linestyles='solid',colors='yellow',transform=ccrs.PlateCarree())
            ax.contour(mlon,mlat,ptas2,[-2], linestyles='solid',colors='purple',transform=ccrs.PlateCarree())
            ax.coastlines()
            ax.set_extent((-180,180,plat,90),crs=ccrs.PlateCarree())
            gl=ax.gridlines(crs=ccrs.PlateCarree(),draw_labels=True,linewidth=0.5,color='gray',y_inline=False)
            gl.ylocator=mticker.FixedLocator([])
            ...
            fig.savefig('%s/djf+jja.dp%02d%s.%s.%s.hl.frz.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
        ...
        fig.savefig('%s/djf+jja.ddp%02d%s.%s.%s.hl.frz.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
    ...
    ax.contour(mlon,mlat,ptas1,[0],transform=ccrs.PlateCarree())
    ax.contour(mlon,mlat,ptas2,[0],transform=ccrs.PlateCarree())
    ax.coastlines()
    ax.set_title(r'%s %s DJF+JJA' % (md.upper(),fo.upper()),fontsize=16)
    cb=fig.colorbar(clf,location='bottom',aspect=50)
    cb.ax.tick_params(labelsize=16)
    cb.set_label(label=r'$\Delta \delta %s$ (%s)'%(vnlb,unlb),size=16)
    fig.savefig('%s/winter.ddp%02d%s.%s.%s.frz.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
