...
def ymax(vn):
    d={ 'hfls':         [-3,1],
        ...
        'oopef_fixmsm': [-0.05,0.05],
        'oopef_rddsm':  [-0.05,0.05],
        'oopef_rbc':    [-0.05,0.05],
            }
    return d[vn]
...
def vstr(vn):
    d={ 'ooplh':        r'$BC_{all}$',
        ...
        'oopef_fixasm': r'$BC_{hist}$, $\Delta\delta SM=0$',
        'oopef_mtr':    r'$\frac{\mathrm{d}LH}{\mathrm{d}SM}_{hist}\Delta SM$',
            }
    return d[vn]
...
def plot(re):
    ...
    if not xlb:
        ax.set_xticklabels([])
    fig.savefig('%s/line.sc.ddp%02d%s.%s%s.ah.avg.%s.png' % (odir,p,varnc,fo,fstr,re), format='png', dpi=600)
    fig.savefig('%s/line.sc.ddp%02d%s.%s%s.ah.avg.%s.pdf' % (odir,p,varnc,fo,fstr,re), format='pdf', dpi=600)
