...
def plot():
    ...
    def load_mmm(varn,varnp):
        idir = '/project/amp02/miyawaki/data/p004/cmip6/%s/%s/%s/%s' % (se,fo,md,varn)
        ds=xr.open_dataset('%s/fddpc.%s_%s_%s.%s.nc' % (idir,varn,his,fut,se))
        fddpvn=ds[varnp]
        pct=ds['percentile']
        gpi=ds['gpi']
        return fddpvn
    ...
    fddpvn1=100*load_mmm(varn1,varnp)
    pct=fddpvn1['percentile']
    if varn1=='ti_ev':
        fddpvn1=-fddpvn1
    ...
    def load_vn(idir0):
        fddpvne=xr.open_dataarray('%s/fddpc.%s_%s_%s.%s.nc' % (idir0,varn1,his,fut,se))
        if varn1=='pr': fddpvne=86400*fddpvne
        return fddpvne
    ...
    fddpvne=[load_vn(idir0) for idir0 in tqdm(idirs)]
    fddpvne=xr.concat(fddpvne,'model')

    # remap to lat x lon
    tvn1=remap(tvn1,gr)
    fddpvn1=remap(fddpvn1,gr)
    fddpvne=eremap(fddpvne,gr)
    ...
    tvn1=tvn1*aagl
    fddpvn1=fddpvn1*aagl
    fddpvne=fddpvne*aagl

    [mlat,mlon] = np.meshgrid(gr['lat'], gr['lon'], indexing='ij')
    awgt=np.cos(np.deg2rad(mlat)) # area weight

    # make sure nans are consistent
    nidx=np.isnan(fddpvn1)
    for imd in range(fddpvne.shape[0]):
        nidx=np.logical_or(np.isnan(nidx),np.isnan(fddpvne[imd,...]))
    tvn1[nidx]=np.nan
    fddpvn1[nidx]=np.nan
    fddpvne[:,nidx]=np.nan

    # annual mean zonal mean
    annz=np.nanmean(fddpvn1,axis=(0)) # keep zonal var
    ann1=np.nanmean(fddpvn1,axis=(0,-1))
    anne=np.nanmean(fddpvne,axis=(1,-1))
    ...
    def selreg(re):
        ah=make_ah(re)
        # select region
        ahfddp1,kidx=regsla(fddpvn1,gr,ah)
        tvn,_=regsla(tvn1,gr,ah)
        ahfddpe=eregsla(fddpvne,gr,ah,kidx)
        # weights
        ahw=regsl2d(awgt,ah,kidx)
        # area weighted mean
        ahfddpg=ahfddp1.copy()
        ahfddp1=np.sum(ahw*ahfddp1,axis=-1)/np.sum(ahw)
        ahfddpe=np.sum(ahw*ahfddpe,axis=-1)/np.sum(ahw)
        tvn=np.sum(ahw*tvn,axis=-1)/np.sum(ahw)
        return ahfddp1,ahfddpe,tvn

    tr_ahfddp1,tr_ahfddpe,tr_tvn=selreg('tr')
    et_ahfddp1,et_ahfddpe,et_tvn=selreg('et')
    ml_ahfddp1,ml_ahfddpe,ml_tvn=selreg('ml')
    hl_ahfddp1,hl_ahfddpe,hl_tvn=selreg('hl')
    ...
    #         al=np.ones(ahfddpe.shape[1])
    #         for ig in tqdm(range(ahfddpe.shape[1])):
    #             sa=ahfddpe[:,ig]
    #             bs=[np.nanmean(resample(sa)) for _ in range(nbs)]
    #             bs=np.array(bs)
    #             pl,pu=np.percentile(bs,100*np.array([alc/2,1-alc/2]))
    ...
    # regions
    tr_ahfddp1=np.nanmean(tr_ahfddp1,axis=0)
    et_ahfddp1=np.nanmean(et_ahfddp1,axis=0)
    ml_ahfddp1=np.nanmean(ml_ahfddp1,axis=0)
    hl_ahfddp1=np.nanmean(hl_ahfddp1,axis=0)
    fig,ax=plt.subplots(figsize=(4,3),constrained_layout=True)
    ax.plot(pct,tr_ahfddp1,color='tab:orange',label='Tropics')
    ax.plot(pct,et_ahfddp1,color='tab:blue',label='Extratropics')
    ...
    fig.savefig('%s/ann.%s.%s%s.ah.pct.sign.f.png' % (odir1,varn1,fo,fstr), format='png', dpi=600)
    fig.savefig('%s/ann.%s.%s%s.ah.pct.sign.f.pdf' % (odir1,varn1,fo,fstr), format='pdf', dpi=600)
