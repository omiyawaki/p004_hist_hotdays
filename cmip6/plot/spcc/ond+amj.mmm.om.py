...
def plot(vn):
    ...
    def selmon(ddpvn):
        ...
        return ddpvno,ddpvna

    ddpvnlmo,ddpvnlma=selmon(ddpvnlm)
    ...
    llddpvnlmo,llddpvnlma=remapll(ddpvnlmo,'lm'),remapll(ddpvnlma,'lm')
    ...
    llddpvnlmo,llddpvnlma=replon(llddpvnlmo),replon(llddpvnlma)
    ...
    llddpvnlm=selhemi(llddpvnlmo,llddpvnlma)
    llddpvnom=selhemi(llddpvnomo,llddpvnoma)

    # combine land and ocean
    llddpvn=np.nan_to_num(llddpvnlm)+np.nan_to_num(llddpvnom)
    ...
    if tr:
        ...
        fig.savefig('%s/ond+amj.ddp%02d%s.%s.%s.tr.om.pdf' % (odir,p,vn,fo,fut), format='pdf', dpi=dpi)
        fig.savefig('%s/ond+amj.ddp%02d%s.%s.%s.tr.om.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
    ...
    if nhhl:
        ...
        cb.set_label(label=r'$\Delta \delta %s$ (%s)'%(varnlb(vn),unitlb(vn)),size=16)
        fig.savefig('%s/ond+amj.ddp%02d%s.%s.%s.nhhl.om.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
    ...
    cb.ax.tick_params(labelsize=16)
    cb.set_label(label=r'$\Delta \delta %s$ (%s)'%(varnlb(vn),unitlb(vn)),size=16)
    fig.savefig('%s/ond+amj.ddp%02d%s.%s.%s.om.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
