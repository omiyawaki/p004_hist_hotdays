...
def plot(re,md):
    ...
    def binned(vn,w,idx):
        ...
        return bvn
    pamp=np.nanpercentile(amp,lpamp)
    iamp=np.digitize(amp,pamp)
    ahddp1=binned(ahddp1,ahw,iamp)
    ...
    if md=='mmm':
        ...
        def ebinned(vn,w,idx):
            ...
            return bvn
        ahddpe=ebinned(ahddpe,ahw,iamp)
    ...
    fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.amp.sign.%s.png' % (odir1,p,varn1,fo,fstr,re), format='png', dpi=600,backend='pgf')
    fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.amp.sign.%s.pdf' % (odir1,p,varn1,fo,fstr,re), format='pdf', dpi=600,backend='pgf')
