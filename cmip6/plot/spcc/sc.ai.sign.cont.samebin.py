...
def plot(re):
    ...
    def binned(vn,w,idx,nb):
        ...
        for im in range(12):
            ...
            svn[im,:]=[np.sqrt(np.nansum(w[idx==i]*(vn[im,idx==i]-bvn[im,i-1])**2)/np.nansum(w[idx==i])) for i in 1+np.arange(nb)]
        return bvn,svn

    # bin into 12
    def ebinned(vn,w,idx,nb):
        bvn=np.empty([vn.shape[0],12,nb])
        for imd in range(vn.shape[0]):
            for im in range(12):
                bvn[imd,im,:]=[np.nansum(w[idx==i]*vn[imd,im,idx==i])/np.nansum(w[idx==i]) for i in 1+np.arange(nb)]
        return bvn

    mai[np.isinf(mai)]=np.nan

    # bin grids before separating into continents
    ...
    for ct in lct:
        try:
            ...
            if bstype=='bins':
                ...
                if md=='mmm':
                    cahddpe=ebinned(cahddpe,ahw,ciai,nb)
                    # bootstrap
                    s=cahddpe.shape
            ...
            cahddp1,stdev=binned(cahddp1,ahw,ciai,nb)

            mon=range(12)
            [cahmmon,cahmbin] = np.meshgrid(mon,mppai, indexing='ij')
            ...
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.ai.sign.%s.%s.samebin.png' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='png', dpi=600,backend='pgf')
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.ai.sign.%s.%s.samebin.pdf' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='pdf', dpi=600,backend='pgf')

            # plot stdev
            fig=plt.figure(figsize=fs)
            divider=Divider(fig, (0, 0, 1, 1), h, v, aspect=False)
            ax=fig.add_axes(divider.get_position(),axes_locator=divider.new_locator(nx=1, ny=1))
            clf=ax.pcolormesh(cahmmon,cahmbin,stdev,vmin=-vmax(varn1),vmax=vmax(varn1),cmap=cmap(varn1))
            # ax.set_title()
            ax.text(0.5,1.05,retname(ct[0]),c=ctitle(varn1),ha='center',va='center',transform=ax.transAxes)
            ax.xaxis.set_minor_locator(MultipleLocator(1))
            ax.set_xticks(np.arange(1,11+2,2))
            ...
            if showcb:
                ...
                if titleoverride:
                    ...
                    cb.set_label('$\sigma(\Delta\delta %s^{%02d})$ (%s)'%(varnlb(varn1),p,unitlb(varn1)))
            fig.savefig('%s/stdev.sc.ddp%02d%s.%s%s.ah.ai.sign.%s.%s.samebin.png' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='png', dpi=600,backend='pgf')
            fig.savefig('%s/stdev.sc.ddp%02d%s.%s%s.ah.ai.sign.%s.%s.samebin.pdf' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='pdf', dpi=600,backend='pgf')
