...
def plot(re,md):
    ...
    def binned(vn,w,idx):
        ...
        return bvn
    pamp=np.nanpercentile(amp,lpamp,axis=1)
    pamp=np.transpose(pamp)
    iamp=[np.digitize(amp[i,:],pamp0) for i,pamp0 in enumerate(pamp)]
    ahddpe=[binned(ahddpe[i,...],ahw[i,:],iamp0) for i,iamp0 in enumerate(iamp)]
    ahddpe=np.stack(ahddpe,axis=0)

    # multimodel mean
    ahddp1=np.nanmean(ahddpe,axis=0)
    ...
    fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.amp.oo.sign.%s.png' % (odir1,p,varn1,fo,fstr,re), format='png', dpi=600,backend='pgf')
    fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.amp.oo.sign.%s.pdf' % (odir1,p,varn1,fo,fstr,re), format='pdf', dpi=600,backend='pgf')
