...
def plot(re):
    ...
    if showcb:
        ...
        if titleoverride:
            ...
            cb.set_label('$\Delta %s$ (%s)'%(varnlb(varn1),unitlb(varn1)))
    fig.savefig('%s/sc.%s.%s%s.ah.pct.sign.%s.a.png' % (odir1,varn1,fo,fstr,re), format='png', dpi=600,backend='pgf')
    fig.savefig('%s/sc.%s.%s%s.ah.pct.sign.%s.a.pdf' % (odir1,varn1,fo,fstr,re), format='pdf', dpi=600,backend='pgf')
