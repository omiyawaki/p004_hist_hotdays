...
def plot(re,md):
    ...
    idirs=[load_idir(md0,varn) for md0 in lmd]
    idirs1=[load_idir(md0,varn1) for md0 in lmd]

    # remap to lat x lon
    ddpvn=[load_vn(idir0,varn) for idir0 in tqdm(idirs)]
    ddpvne=[load_vn(idir0,varn1) for idir0 in tqdm(idirs1)]
    ddpvn=xr.concat(ddpvn,'model')
    ddpvne=xr.concat(ddpvne,'model')
    ddpvn=eremap(ddpvn,gr)
    ...
    amp=np.nanmax(ddpvn,axis=1)-np.nanmin(ddpvn,axis=1)

    [mlat,mlon] = np.meshgrid(gr['lat'], gr['lon'], indexing='ij')
    awgt=np.cos(np.deg2rad(mlat)) # area weight
    awgt=np.tile(awgt,(len(lmd),1,1))
    ...
    # select region
    ahddp,nfilt=eregsla(ddpvn,gr,ah,nfilt=None)
    ahddpe,_=eregsla(ddpvne,gr,ah,nfilt=nfilt)
    amp=regsl3d(amp,ah,nfilt)
    # weights
    ahw=regsl3d(awgt,ah,nfilt)
    ...
    for ct in lct:
        try:
            ...
            mask2d=regsl2d(mask,ah,np.squeeze(nfilt))
            mask=np.tile(mask,(12,1,1))
            mask=mregsla(mask,gr,ah,nfilt)
            camp=mask2d*amp
            cahddpe=mask*ahddpe
            pamp=np.nanpercentile(camp,lpamp,axis=1)
            pamp=np.transpose(pamp)
            iamp=[np.digitize(camp[i,:],pamp0) for i,pamp0 in enumerate(pamp)]
            cahddpe=[binned(ahddpe[i,...],ahw[i,:],iamp0) for i,iamp0 in enumerate(iamp)]
            cahddpe=np.stack(cahddpe,axis=0)

            # multimodel mean
            ahddp1=np.nanmean(cahddpe,axis=0)
            ...
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.amp.oo.sign.%s.%s.png' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='png', dpi=600,backend='pgf')
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.amp.oo.sign.%s.%s.pdf' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='pdf', dpi=600,backend='pgf')
