...
varn4='ooplh_rbcsm'
varnp4='ooplh'
varnc='%s+%s+%s+%s'%(varn1,varn2,varn3,varn4)
...
def cline(vn):
    d={ 'hfls':         'tab:blue',
        'ooplh':        'k',
        'ooplh_msm':    'blue',
        'ooplh_fixmsm': 'olivedrab',
        'ooplh_fixasm': 'blue',
        'ooplh_fixbc':  'slateblue',
        'ooplh_dbc':    'chocolate',
        'ooplh_rbcsm':  'teal',
        'ooplh_rddsm':  'goldenrod',
        'ooplh_mtr':    'blue',
            }
    return d[vn]
...
def plot(re):
    ...
    nidx=np.logical_or(np.isnan(nidx),np.isnan(ddpvn4))
    ddpvn[nidx]=np.nan
    ddpvn1[nidx]=np.nan
    ddpvn2[nidx]=np.nan
    ddpvn3[nidx]=np.nan
    ...
    ahddp4,_=binned(ahddp4,ahw,ahidxrs)
    # repeat for DdT
    ...
    ahddp4=[ahddp4[i,i] for i in mon]
    ...
    ax.plot(mon,ahddp4,color=cline(varn4))
    # ax.set_title(tstr)
