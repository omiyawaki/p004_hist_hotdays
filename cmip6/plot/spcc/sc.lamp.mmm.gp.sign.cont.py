...
def plot(re):
    ...
    def ebinned(vn,w,idx):
        bvn=np.empty([vn.shape[0],12,12])
        bw=np.zeros(13)
        for imd in range(vn.shape[0]):
            for im in range(12):
                bvn[imd,im,:]=[np.nansum(w[im,idx==i]*vn[imd,im,idx==i])/np.nansum(w[im,idx==i]) for i in range(12)]
                bw[im+1]=np.nansum(w[im,idx==im])+bw[im]
        return bvn,bw/bw[-1]
    ...
    for ct in lct:
        try:
            ...
            cahw=mask*ahw
            cahidxrs=mask[0,:]*ahidxrs
            cahddp,cahcw=binned(cahddp,cahw,cahidxrs)
            cahddp1,_=binned(cahddp1,cahw,cahidxrs)
            cahddpe,_=ebinned(cahddpe,cahw,cahidxrs)
            ...
            for ig in tqdm(range(cahddpe.shape[1])):
                sa=cahddpe[:,ig]
                bs=[np.nanmean(resample(sa)) for _ in range(nb)]
                bs=np.array(bs)
            ...
            cahcwmp=1/2*(cahcw[1:]+cahcw[:-1])

            mon=range(12)
            [cahmbin,cahmmon] = np.meshgrid(cahcwmp,mon,indexing='ij')

            # plot gp vs seasonal cycle of varn1 PCOLORMESH
            fig=plt.figure(figsize=fs)
            divider=Divider(fig, (0, 0, 1, 1), h, v, aspect=False)
            ax=fig.add_axes(divider.get_position(),axes_locator=divider.new_locator(nx=1, ny=1))
            clf=ax.pcolormesh(cahmmon,cahmbin,np.transpose(cahddp1),vmin=-vmax(varn1),vmax=vmax(varn1),cmap=cmap(varn1))
            hatch=plt.fill_between([-0.5,11.5],0,1,hatch='///////',color='none',edgecolor='gray',linewidths=0.3)
            ax.pcolormesh(cahmmon,cahmbin,np.ma.masked_where(al==0,np.transpose(cahddp1)),vmin=-vmax(varn1),vmax=vmax(varn1),cmap=cmap(varn1))
            # ax.set_title()
            ...
            x=np.arange(12)
            y=cahcwmp
            z=np.diag(np.diag(np.ones_like(cahddp)))
            # make dense grid
            scale=100
            ...
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.gp.sign.%s.%s.png' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='png', dpi=600,backend='pgf')
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.gp.sign.%s.%s.pdf' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='pdf', dpi=600,backend='pgf')
