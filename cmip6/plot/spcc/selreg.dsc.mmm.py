...
def set_cticks(varn):
    cticks={    'tas':      np.arange(-8,8.1,0.25),
                'pr':       np.arange(-3,3.1,0.25),  
                }
    return cticks[varn]
def set_cmap(varn):
    cmap={  'tas':  'RdBu_r',
...
def plot(varn,relb,foi=fo1,foo=fo1,yr=his):
    ...
    for m in tqdm(range(12)):
        ...
        clf=ax[m].contourf(mlon, mlat, llcm[m,...], cticks,extend='both', vmax=cticks[-1], vmin=cticks[0], transform=ccrs.PlateCarree(), cmap=set_cmap(varn))
        ax[m].coastlines()
...
for relb in lrelb:
    retn=retname(relb)
    rell=window(relb)
    plot(varn,relb,foi=fod,foo=fod,yr=yd)
