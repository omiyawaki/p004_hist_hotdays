...
def plot(vn):
    ...
    # warming
    ds=xr.open_dataset('%s/ddp.%s_%s_%s.%s.nc' % (idir,vn,his,fut,se))
    ddpvn=ds[vn]
    pct=ds['percentile']
    gpi=ds['gpi']
    ...
    for m in tqdm(range(12)):
        ...
        # else:
        clf=ax[m].contourf(mlon, mlat, llhotd[m,...]-llmean[m,...], np.arange(-vm,vm+dvm,dvm),extend='both', vmax=vm, vmin=-vm, transform=ccrs.PlateCarree(), cmap='RdBu_r')
        ax[m].coastlines()
        ax[m].set_title(r'%s' % (monname(m)),fontsize=16)
        fig.savefig('%s/ddp%02d%s.%s.png' % (odir,p,vn,fo), format='png', dpi=300)
