...
def plot(re):
    ...
    for ct in lct:
        try:
            ...
            pmx=np.nanpercentile(cmx,lpmx)
            imx=np.digitize(cmx,pmx)
            cahddp1=binned(cahddp1,ahw,imx)
            cahddpe=ebinned(cahddpe,ahw,imx)
            ...
            mon=range(12)
            [cahmmon,cahmbin] = np.meshgrid(mon,mppmx, indexing='ij')
            ...
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.max.sign.%s.%s.png' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='png', dpi=600,backend='pgf')
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.max.sign.%s.%s.pdf' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='pdf', dpi=600,backend='pgf')
