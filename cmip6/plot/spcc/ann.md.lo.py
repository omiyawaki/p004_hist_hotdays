...
def loadmvn(md,fo,yr,vn,ocn=False):
    idir = '/project/amp02/miyawaki/data/p004/cmip6/%s/%s/%s/%s' % (se,fo,md,vn)
    if ocn:
        ds=xr.open_dataset('%s/m.om.%s_%s.%s.nc' % (idir,vn,yr,se))
    else:
...
def loadpvn(md,fo,yr,vn,ocn=False):
    idir = '/project/amp02/miyawaki/data/p004/cmip6/%s/%s/%s/%s' % (se,fo,md,vn)
    if ocn:
        ds=xr.open_dataset('%s/pc.om.%s_%s.%s.nc' % (idir,vn,yr,se))
    else:
...
def plot(md):
    ...
    # land warming
    pvn1,pct,gpi=loadpvn(md,fo1,his,vn,ocn=False)
    pvn2,_,_=loadpvn(md,fo2,fut,vn,ocn=False)
    mvn1,_=loadmvn(md,fo1,his,vn,ocn=False)
    mvn2,_=loadmvn(md,fo2,fut,vn,ocn=False)
    dvn=mvn2-mvn1
    dpvn=pvn2-pvn1
    ...
    # ocean warming
    opvn1,_,ogpi=loadpvn(md,fo1,his,vn,ocn=True)
    opvn2,_,_=loadpvn(md,fo2,fut,vn,ocn=True)
    omvn1,_=loadmvn(md,fo1,his,vn,ocn=True)
    omvn2,_=loadmvn(md,fo2,fut,vn,ocn=True)
    odvn=omvn2-omvn1
    odpvn=opvn2-opvn1
    oddpvn=odpvn-np.transpose(odvn.data[...,None],[0,2,1])
    odpvn=odpvn.sel(percentile=p).squeeze()
    oddpvn=oddpvn.sel(percentile=p).squeeze()
