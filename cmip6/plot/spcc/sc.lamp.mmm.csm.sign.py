...
bsm=np.arange(-10,10+1,1) # SM bins
mpbsm=1/2*(bsm[1:]+bsm[:-1])
...
def plot(re):
    ...
    sm=load_vn(varn,fo1,his,px='anom.m') # historical AI
    ...
    def binned(vn,w,idx,nb):
        ...
        return bvn
    ism=np.digitize(msm,bsm)
    ahddp1=binned(ahddp1,ahw,ism,len(bsm)-1)
    ...
    def ebinned(vn,w,idx,nb):
        ...
        return bvn
    ahddpe=ebinned(ahddpe,ahw,ism,len(bsm)-1)
    ...
    mon=range(12)
    [ahmmon,ahmbin] = np.meshgrid(mon,mpbsm, indexing='ij')
    ...
    # ax.set_xticklabels(np.arange(2,12+2,2))
    if ylb:
        ax.set_ylabel('$\overline{SM}-SM_c$ (kg m$^{-2}$)')
    else:
        ax.set_yticklabels([])
    if not xlb:
        ax.set_xticklabels([])
    ax.set_xlim([-0.5,11.5])
    ax.set_ylim([bsm.min(),bsm.max()])
    if showcb:
    ...
    fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.sm.sign.%s.png' % (odir1,p,varn1,fo,fstr,re), format='png', dpi=600,backend='pgf')
    fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.sm.sign.%s.pdf' % (odir1,p,varn1,fo,fstr,re), format='pdf', dpi=600,backend='pgf')
