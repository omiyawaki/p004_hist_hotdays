...
varn1='tas'
varnp='tas'
se = 'sc' # season (sc, djf, mam, jja, son)
...
def plot():
    ...
    def load_vn(varn,fo,byr,px='m'):
        ...
        return xr.open_dataarray('%s/%s.%s_%s.%s.nc' % (idir,px,varn,byr,se)).sel(percentile=selp)

    def load_mmm(varn,varnp):
        idir = '/project/amp02/miyawaki/data/p004/cmip6/%s/%s/%s/%s' % (se,fo,md,varn)
        ds=xr.open_dataset('%s/ddpc.%s_%s_%s.%s.nc' % (idir,varn,his,fut,se))
        ddpvn=ds[varnp]
        ddpvn=ddpvn.sel(percentile=selp)
        pct=ds['percentile']
        gpi=ds['gpi']
        return ddpvn
    ...
    def load_vn(idir0):
        ...
        ddpvne=ddpvne.sel(percentile=selp)
    ...
    # zonal mean
    sc1=np.nanmean(ddpvn1,axis=(-1)).squeeze()
    sce=np.nanmean(ddpvne,axis=(-1)).squeeze()

    # shift seasonality by 6 months in SH
    ssc1=np.roll(sc1,6,axis=0)
    sc1[:,gr['lat']<0]=ssc1[:,gr['lat']<0]
    ...
    #         # bootstrap
    #         s=sce.shape
    #         sce=np.reshape(sce,(s[0],s[1]*s[2]))
    ...
    mon=np.arange(12)+1
    [ahmmon,ahmlat] = np.meshgrid(mon,gr['lat'], indexing='ij')
    ...
    ax.plot(mon,tr_ahddp1,color='tab:orange',label='Tropics')
    ax.plot(mon,et_ahddp1,color='tab:blue',label='Extratropics')
    ax.set_xlabel('Months after winter solstice')
    ax.set_ylabel('$\Delta\delta$ %s (%s)'%(varnlb(varn1),unitlb(varn1)))
    ax.xaxis.set_minor_locator(MultipleLocator(1))
    ax.set_xlim([1,12])
    ax.set_title('$>95$th percentile days')
    ax.legend(frameon=False)
    fig.savefig('%s/lat.%s.%s%s.ah.sc.reg.png'%(odir1,varn1,fo,fstr),format='png',dpi=600)

    # plot gp vs seasonal cycle of varn1 PCOLORMESH
    fig,ax=plt.subplots(figsize=(3,5),constrained_layout=True)
    clf=ax.pcolormesh(ahmmon,ahmlat,sc1,vmin=-vmax(varn1),vmax=vmax(varn1),cmap=cmap(varn1))
    # if bstype=='bins' or md=='mmm':
    #     hatch=plt.fill_between([-0.5,11.5],0,100,hatch='///////',color='none',edgecolor='gray',linewidths=0.3)
    #     ax.pcolormesh(ahmlat,ahmpct,np.ma.masked_where(np.transpose(al==0),np.transpose(sc1)),vmin=-vmax(varn1),vmax=vmax(varn1),cmap=cmap(varn1))
    # ax.contour(ahmlat,ahmpct,np.transpose(sc1),[0],linewidths=0.5,colors='gray')
    # ax.set_title()
    ax.text(0.5,1.05,'$>95$th percentile days',c=ct(varn1),ha='center',va='center',transform=ax.transAxes)
    ax.set_xlabel('Months after winter solstice')
    ...
    fig.savefig('%s/lat.%s.%s%s.ah.sc.sign.png' % (odir1,varn1,fo,fstr), format='png', dpi=600)
    fig.savefig('%s/lat.%s.%s%s.ah.sc.sign.pdf' % (odir1,varn1,fo,fstr), format='pdf', dpi=600)
