...
def plot(re,md):
    ...
    for ct in lct:
        try:
            ...
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.max.oo.%s.%s.png' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='png', dpi=600,backend='pgf')
            fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.max.oo.%s.%s.pdf' % (odir1,p,varn1,fo,fstr,re,ct[0]), format='pdf', dpi=600,backend='pgf')
