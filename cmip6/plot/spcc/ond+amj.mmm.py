...
def plot(vn):
    ...
    # ond and amj means
    dvno  =np.nanmean(dvn.data[9:12,:]  ,axis=0)
    dpvno =np.nanmean(dpvn.data[9:12,:] ,axis=0)
    ddpvno=np.nanmean(ddpvn.data[9:12,:],axis=0)
    dvna  =np.nanmean(dvn.data[3:6,:]  ,axis=0)
    dpvna =np.nanmean(dpvn.data[3:6,:] ,axis=0)
    ...
    fig.savefig('%s/ond+amj.dp%02d%s.%s.%s.png' % (odir,p,vn,fo,fut), format='png', dpi=dpi)
    ...
    fig.savefig('%s/ond+amj.d%s.%s.%s.png' % (odir,vn,fo,fut), format='png', dpi=dpi)
