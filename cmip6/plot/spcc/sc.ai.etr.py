...
def plot(re):
    ...
    xahc.to_netcdf('%s/sc.%s.%s_%s.%s.nc'%(ddir,varnc,his,fut,re))
    ...
    ax.set_ylim([0,100])
    fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.ai.%s.png' % (odir1,p,varnc,fo,fstr,re), format='png', dpi=600)
    fig.savefig('%s/sc.ddp%02d%s.%s%s.ah.ai.%s.pdf' % (odir1,p,varnc,fo,fstr,re), format='pdf', dpi=600)

    # save colorbar only
    fig,ax=plt.subplots(figsize=(1,2))
    cb=plt.colorbar(clf,ax=ax,location='right')
    ax.remove()
    plt.savefig('%s/sc.ddp%02d%s.%s.ah.ai.cb.pdf' % (odir1,p,varnc,fo), format='pdf', dpi=600,bbox_inches='tight')
