...
for flag in lflag:
    ...
    clf=ax.contourf(mpct, mlbn, np.transpose(dat), np.arange(-1.5,1.5+1,0.1),extend='both', vmax=1.5, vmin=-1.5, cmap='RdBu_r')
    tname=r'%s' % (md.upper())
    if flag['landonly']:
        tname='%s, Land'%tname
    if flag['troponly']:
    ...
    ax.set_ylabel('$\Delta T^x_\mathrm{2\,m}$ percentile')
