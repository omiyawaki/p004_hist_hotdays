...
def plot(varn,relb,ll1,ll2):
    ...
    fig.savefig('%s/cd.%s.%s.%s.png' % (odir,varn,fo1,relb), format='png', dpi=600)
...
for relb in lrelb:
    for varn in lvarn:
        ...
        # ll data
        if md=='mmm':
            ds=xr.open_dataset('%s/cd.%s_%s.%s.nc' % (idir1,varn,his,se))
            ll1=ds[varn]
            ll2=xr.open_dataarray('%s/cd.%s_%s.%s.nc' % (idir2,varn,fut,se))
        else:
            ds=xr.open_dataset('%s/lm.%s_%s.%s.nc' % (idir1,varn,his,se))
            ll1=ds[varn]
