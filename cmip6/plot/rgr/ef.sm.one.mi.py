...
for se in lse:
    ...
    fig,ax=plt.subplots(nrows=5,ncols=4,figsize=(11,11))
    ax=ax.flatten()
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        bnd=[np.minimum(np.nanmin(th),np.nanmin(l2)),np.maximum(np.nanmax(th),np.nanmax(l2))]
        lsm=np.linspace(bnd[0],bnd[1],500)
        ...
        ax[imd].axvline(th[0],color='k',linewidth=0.5)
        ax[imd].axvline(th[1],color='k',linewidth=0.5)
        ax[imd].scatter(l2,l1,c=pdf,s=0.5,label=hrn[lhr])
        xlim=ax[imd].get_xlim()
        ylim=ax[imd].get_ylim()
        ax[imd].plot(lsm,llh,'-',color='tab:red')
        ax[imd].set_xlim(xlim)
        ax[imd].set_ylim(ylim)
        ax[imd].set_title(r'%s' % (md.upper()))
        ax[imd].set_xlabel(r'$SM$ (kg m$^{-2}$)')
        ax[imd].set_ylabel(r'$LH/R_{SFC}$ (unitless)')
        plt.legend()
        plt.tight_layout()
        plt.savefig('%s/scatter.rgr.%s.kde.%s.%+05.1f.%+05.1f.%s.pdf' % (odir,rmethod,varn,la,lo,se), format='pdf', dpi=128)
    plt.close()
