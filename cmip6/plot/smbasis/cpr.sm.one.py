...
for nt in lnt:
    ...
    for se in lse:
        ...
        for md in tqdm(lmd):
            ...
            # ax.fill_between(sm1,cpr1-sd1,cpr1+sd1,color='tab:blue',ec=None,alpha=0.3)
            # ax.fill_between(sm2,cpr2-sd2,cpr2+sd2,color='tab:orange',ec=None,alpha=0.3)
            ax.plot(sm1,cpr1,color='tab:blue',label='1980-2000')
            ax.plot(sm2,cpr2,color='tab:orange',label='2080-2100')
            ax.set_xlim([0,50])
            ax.set_title(r'%s %s [%+05.1f,%+05.1f]' % (se.upper(),md.upper(),la,lo))
            ax.set_xlabel(r'$SM$ (kg m$^{-2}$)')
            ax.set_ylabel(r'%g-day $P_{SM}$ (mm d$^{-1}$)'%nt)
