...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        ax[imd].fill_between(sm1,tas1-sd1,tas1+sd1,color='tab:blue',ec=None,alpha=0.3)
        ax[imd].fill_between(sm2,tas2-sd2,tas2+sd2,color='tab:orange',ec=None,alpha=0.3)
        ax[imd].plot(sm1,tas1,color='tab:blue',label='1980-2000')
        ax[imd].plot(sm2,tas2,color='tab:orange',label='2080-2100')
        ax[imd].set_title(r'%s' % (md.upper()))
        ax[imd].set_xlim([0,50])
        ax[imd].set_ylim([290,320])
        ax[imd].set_xlabel(r'$SM$ (kg m$^{-2}$)')
        ax[imd].set_ylabel(r'$T_\mathrm{2\,m}$ (K)')
        plt.legend()
        plt.tight_layout()
