...
for se in lse:
    ...
    [ef1,sm1,sd1,eef1,esd1] = pickle.load(open(fn1,'rb'))
    [ef2,sm2,sd2,eef2,esd2] = pickle.load(open(fn2,'rb'))
    delef=ef2-ef1
    asd1=np.nanstd(eef1,axis=0)
    asd2=np.nanstd(eef2,axis=0)
    delsd=np.sqrt(asd1**2+asd2**2)
    ...
    ax.set_ylabel(r'$EF_{SM}$ (unitless)')
    plt.legend(frameon=False,loc='upper left')
    plt.tight_layout()
    plt.savefig('%s/%s.%+05.1f.%+05.1f.%s.pdf' % (odir,varn,la,lo,se), format='pdf', dpi=300)
    plt.close()

    # plot pdfs of delta var (hist and fut)
    fig,ax=plt.subplots(figsize=(5,4))
    ax.axhline(0,color='k',linewidth=0.5)
    # ax.fill_between(sm1,ef1-sd1,ef1+sd1,color='tab:blue',ec=None,alpha=0.3)
    # ax.fill_between(sm2,ef2-sd2,ef2+sd2,color='tab:orange',ec=None,alpha=0.3)
    ax.fill_between(sm1,delef-delsd,delef+delsd,color='k',ec=None,alpha=0.3)
    ax.plot(sm1,delef,color='k',label='F-H')
    ax.set_xlim([0,50])
    ax.set_ylim([0,2])
    ax.set_title(r'%s %s [%+05.1f,%+05.1f]' % (se.upper(),md.upper(),la,lo))
    ax.set_xlabel(r'$SM$ (kg m$^{-2}$)')
    ...
    plt.legend(frameon=False,loc='upper left')
    plt.tight_layout()
    plt.savefig('%s/d%s.%+05.1f.%+05.1f.%s.pdf' % (odir,varn,la,lo,se), format='pdf', dpi=300)
    plt.close()
