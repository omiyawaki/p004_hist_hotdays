...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        [ef1,sm1] = pickle.load(open(fn1,'rb'))
        [ef2,sm2] = pickle.load(open(fn2,'rb'))
        ...
        ax[imd].set_xlim([-30,20])
        ax[imd].set_ylim([0,1.5])
        ax[imd].set_xlabel(r'$\delta SM$ (kg m$^{-2}$)')
        ax[imd].set_ylabel(r'$EF_{SM}$ (unitless)')
        plt.legend()
        plt.tight_layout()
        plt.savefig('%s/b.%s.%+05.1f.%+05.1f.%s.pdf' % (odir,varn,la,lo,se), format='pdf', dpi=128)
    plt.close()
