...
for nt in lnt:
    ...
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            [cpr1,sm1,sd1] = pickle.load(open(fn1,'rb'))
            [cpr2,sm2,sd2] = pickle.load(open(fn2,'rb'))
            ...
            ax[imd].fill_between(sm1,cpr1-sd1,cpr1+sd1,color='tab:blue',ec=None,alpha=0.3)
            ax[imd].fill_between(sm2,cpr2-sd2,cpr2+sd2,color='tab:orange',ec=None,alpha=0.3)
            ax[imd].plot(sm1,cpr1,color='tab:blue',label='1980-2000')
            ax[imd].plot(sm2,cpr2,color='tab:orange',label='2080-2100')
            ax[imd].set_title(r'%s' % (md.upper()))
            ax[imd].set_xlim([0,50])
            ax[imd].set_ylim([0,30])
            ax[imd].set_xlabel(r'$SM$ (kg m$^{-2}$)')
            ax[imd].set_ylabel(r'%g-day $P_{SM}$ (mm d$^{-1}$)'%nt)
            plt.legend()
            plt.tight_layout()
