...
from utils import corr2d,monname
...
for args in largs:
    ...
    if mmm:
        ...
        if ann:
            ...
            abm=np.vstack(stack)
            pdf=kde(abm)
            ax.scatter(86400*mi2,mi1,s=0.1,c=pdf)
            ax.set_title(r'%s, ANN' % (tname),fontsize=16)
            ax.set_xlabel('$\Delta\overline{P}$ (mm d$^{-1}$)')
            ...
            for mon in range(12):
                ...
                mi1=mi1[~np.isnan(mi1)]
                mi2=mi2[~np.isnan(mi2)]
                ax[mon].scatter(86400*mi2,mi1,s=0.1,c='k')
                ax[mon].set_title(r'%s' % (monname(mon)),fontsize=16)
                ax[mon].set_xlabel('$\Delta\overline{P}$ (mm d$^{-1}$)')
        ...
        if ann:
            ...
            ax[-1].set_xlabel('$\Delta\overline{P}$ (mm d$^{-1}$)')
            ...
            for imd,md in enumerate(lmd):
                ...
                ax[imd].set_xlabel('$\Delta\overline{P}$ (mm d$^{-1}$)')
