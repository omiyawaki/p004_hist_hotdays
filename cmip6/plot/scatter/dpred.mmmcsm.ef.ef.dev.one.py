...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            vn2c1 = pickle.load(open('%s/pc%s_%s.%g.%g.%s.dev.pickle' % (idir2c1,varn2,his,ila,ilo,se), 'rb'))
            vn2c2 = pickle.load(open('%s/pc.mmmcsm%s_%s.%g.%g.%s.dev.pickle' % (idir2c2,varn2,fut,ila,ilo,se), 'rb'))
            ...
            if pc==0:
                ...
                ax.set_xlabel(r'$BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s}+\Delta\delta SM^{>%s}_\mathrm{MMM})-BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s})$ (unitless)' % ('F',pc,'F',pc,'H',pc,'H'))
        ...
        plt.tight_layout()
        plt.savefig('%s/scatter.dpc.mmmcsm.%s.%s.%+05.1f.%+05.1f.%s.%s.dev.pdf' % (odir,varn1,varn2,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
