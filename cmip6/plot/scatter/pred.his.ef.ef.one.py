...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            [_,gr] = pickle.load(open('%s/c%s_%s.%s.pickle' % (idir1,varn1,yr,se), 'rb'))
            vn1 = pickle.load(open('%s/pc%s_%s.%g.%g.%s.pickle' % (idir2,varn2,yr,ila,ilo,se), 'rb'))
            vn2 = pickle.load(open('%s/pc.hisbc%s_%s.%g.%g.%s.pickle' % (idir2,varn2,yr,ila,ilo,se), 'rb'))
            ...
            if pc==0:
                ax.set_ylabel(r'$BC(\overline{SM})_\mathrm{%s}$ (unitless)' % (yr))
                ax.set_xlabel(r'$BC_\mathrm{%s}(\overline{SM})_\mathrm{%s}$ (unitless)' % ('1980-2000',yr))
            else:
                ax.set_ylabel(r'$BC(SM^{>%s})_\mathrm{%s}$ (unitless)' % (pc,yr))
                ax.set_xlabel(r'$BC_\mathrm{%s}(SM^{>%s})_\mathrm{%s}$ (unitless)' % ('1980-2000',pc,yr))
        vec=np.linspace(np.min(xy),np.max(xy),2)
        ...
        plt.tight_layout()
        plt.savefig('%s/scatter.pc.hisbc.%s.%s.%+05.1f.%+05.1f.%s.%s.pdf' % (odir,varn1,varn2,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
