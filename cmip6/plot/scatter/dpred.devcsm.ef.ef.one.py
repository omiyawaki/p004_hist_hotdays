...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            vn2c1 = pickle.load(open('%s/pc.devcsm%s_%s.%g.%g.%s.pickle' % (idir2c1,varn2,his,ila,ilo,se), 'rb'))
            vn2c2 = pickle.load(open('%s/pc.devcsm%s_%s.%g.%g.%s.pickle' % (idir2c2,varn2,fut,ila,ilo,se), 'rb'))
            ...
            if pc==0:
                ...
                ax.set_xlabel(r'$BC_\mathrm{%s,MMM}(\overline{SM_\mathrm{%s,MMM}}+\Delta \overline{SM})-BC_\mathrm{%s,MMM}(\overline{SM_\mathrm{%s,MMM}})$ (unitless)' % ('F','H','H','H'))
            else:
                ax.set_ylabel(r'$BC_\mathrm{%s}(SM^{>%s}_\mathrm{%s})-BC_\mathrm{%s}(SM^{>%s}_\mathrm{%s})$ (unitless)' % ('F',pc,'F','H',pc,'H'))
                ax.set_xlabel(r'$BC_\mathrm{%s,MMM}(SM^{>%s}_\mathrm{%s,MMM}+\Delta SM^{>%s})-BC_\mathrm{%s,MMM}(SM^{>%s}_\mathrm{%s,MMM})$ (unitless)' % ('F',pc,'H',pc,'H',pc,'H'))
        ...
        plt.tight_layout()
        plt.savefig('%s/scatter.dpc.devcsm.%s.%s.%+05.1f.%+05.1f.%s.%s.pdf' % (odir,varn1,varn2,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
