...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            vn2c1 = pickle.load(open('%s/pc.mmmhistcsm%s_%s.%g.%g.%s.dev.pickle' % (idir2c1,varn2,his,ila,ilo,se), 'rb'))
            vn2c2 = pickle.load(open('%s/pc.mmmhistcsm%s_%s.%g.%g.%s.dev.pickle' % (idir2c2,varn2,fut,ila,ilo,se), 'rb'))
            ...
            if pc==0:
                ax.set_ylabel(r'$BC_\mathrm{%s}(0)-BC_\mathrm{%s}(0)$ (unitless)' % ('F','H'))
                ax.set_xlabel(r'$BC_\mathrm{%s}(0)-BC_\mathrm{%s}(0)$ (unitless)' % ('F','H'))
            else:
                ax.set_ylabel(r'$BC(SM^{>%s}+\Delta SM^{>%s})-BC(SM^{>%s})$ (unitless)' % (pc,pc,pc))
                ax.set_xlabel(r'$BC(SM^{>%s}_{MMM}+\Delta SM^{>%s})-BC(SM^{>%s}_{MMM})$ (unitless)' % (pc,pc,pc))
                ax.set_ylabel(r'$BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s})-BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s})$ (unitless)' % ('F',pc,'F','H',pc,'H'))
                ax.set_xlabel(r'$BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s,MMM}+\Delta\delta SM^{>%s})-BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s,MMM})$ (unitless)' % ('F',pc,'F',pc,'H',pc,'H'))
        ...
        plt.tight_layout()
        plt.savefig('%s/scatter.dpc.mmmhistcsm.%s.%s.%+05.1f.%+05.1f.%s.%s.dev.pdf' % (odir,varn1,varn2,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
