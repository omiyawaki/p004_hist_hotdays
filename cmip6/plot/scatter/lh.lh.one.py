...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            md=lmd[imd]
            print(md)
            idir1 = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,cl,fo,md,varn)
            idir2 = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,'his','historical',md,varn)

            [vn1, gr] = pickle.load(open('%s/diff_%02d.%s.%s.%s.pickle' % (idir1,pc,his,fut,se), 'rb'))
            [vn2, gr] = pickle.load(open('%s/c%s_%s.%s.pickle' % (idir2,varn,his,se), 'rb'))
            ...
            odir = '/project/amp/miyawaki/plots/p004/cmip6/%s/%s/%s/%s/%s+%s/selpoint/lat_%+05.1f/lon_%+05.1f/' % (se,cl,fo,'mi',varn,varn,la,lo)
            ...
            if pc==0:
                ...
                ax.set_xlabel(r'$\overline{LH}_\mathrm{%s}$ (W m$^{-2}$)' % (his))
            else:
                ax.set_ylabel(r'$LH^{>%s}_\mathrm{%s}-LH^{>%s}_\mathrm{%s}$ (W m$^{-2}$)' % (pc,fut,pc,his))
                ax.set_xlabel(r'$LH^{>%s}_\mathrm{%s}$ (W m$^{-2}$)' % (pc,his))
        m,y0,rv,pv,sterr=linregress(xy[1,:],xy[0,:])
        xv=np.linspace(xy[1,:].min(),xy[1,:].max(),2)
        ax.plot(xv,y0+m*xv,color='tab:red')
        ax.text(0.99,0.99,'$R=%+.2f$'%rv,ha='right',va='top',transform=ax.transAxes)
        plt.legend(frameon=False,bbox_to_anchor=(1,1),loc='upper left',prop={'size':8})
        plt.tight_layout()
        plt.savefig('%s/scatter.%s.%s.%+05.1f.%+05.1f.%s.%s.pdf' % (odir,varn,varn,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
