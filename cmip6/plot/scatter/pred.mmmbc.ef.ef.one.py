...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            [vn1,gr] = pickle.load(open('%s/c%s_%s.%s.pickle' % (idir1,varn1,yr,se), 'rb'))
            vn2 = pickle.load(open('%s/pc%s_%s.%g.%g.%s.pickle' % (idir2,varn2,yr,ila,ilo,se), 'rb'))
            ...
            if pc==0:
                ax.set_ylabel(r'$\overline{EF}_\mathrm{%s}$ (unitless)' % (yr))
                ax.set_xlabel(r'$BC(\overline{SM})_\mathrm{%s}$ (unitless)' % (yr))
            else:
                ax.set_ylabel(r'$EF^{>%s}_\mathrm{%s}$ (unitless)' % (pc,yr))
                ax.set_xlabel(r'$BC(SM^{>%s})_\mathrm{%s}$ (unitless)' % (pc,yr))
        ...
        plt.tight_layout()
        plt.savefig('%s/scatter.pc.%s.%s.%+05.1f.%+05.1f.%s.%s.pdf' % (odir,varn1,varn2,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
