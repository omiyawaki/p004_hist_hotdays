...
varn2='mrsos'
filtppr=False
ppr0=1 # threshold for dry hot days mm/d
...
for args in largs:
    ...
    if mmm:
        ...
        if ann:
            ...
            ax.set_xlabel('$\Delta\delta{SM}^{%02d}$ (kg m$^{-2}$)'%p)
            ...
            for mon in range(12):
                ...
                ax[mon].scatter(mi2,mi1,s=0.1,c=mi3)
                ax[mon].set_title(r'%s' % (monname(mon)),fontsize=16)
                ax[mon].set_xlabel('$\Delta\delta{SM}^{%02d}$ (kg m$^{-2}$)'%p)
        ...
        if ann:
            ...
            ax[-1].set_xlabel('$\Delta\delta{SM}^{%02d}$ (kg m$^{-2}$)'%p)
            ...
            for imd,md in enumerate(lmd):
                ...
                ax[imd].set_xlabel('$\Delta\delta{SM}^{%02d}$ (kg m$^{-2}$)'%p)
