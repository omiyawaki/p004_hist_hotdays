...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            vn2c1 = pickle.load(open('%s/pc.devhistcsm%s_%s.%g.%g.%s.dev.pickle' % (idir2c1,varn2,his,ila,ilo,se), 'rb'))
            vn2c2 = pickle.load(open('%s/pc.devhistcsm%s_%s.%g.%g.%s.dev.pickle' % (idir2c2,varn2,fut,ila,ilo,se), 'rb'))
            ...
            if pc==0:
                ax.set_ylabel(r'$BC_\mathrm{%s}(\overline{\delta SM_\mathrm{%s}})-BC_\mathrm{%s}(\overline{\delta SM_\mathrm{%s}})$ (unitless)' % ('F','F','H','H'))
                ax.set_xlabel(r'$BC_\mathrm{%s,MMM}(\overline{\delta SM_\mathrm{%s}}+\Delta \overline{\delta SM_{MMM}})-BC_\mathrm{%s,MMM}(\overline{\delta SM_\mathrm{%s}})$ (unitless)' % ('F','H','H','H'))
            else:
                ax.set_ylabel(r'$BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s})-BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s})$ (unitless)' % ('F',pc,'F','H',pc,'H'))
                ax.set_xlabel(r'$BC_\mathrm{%s,MMM}(\delta SM^{>%s}_\mathrm{%s}+\Delta \delta SM^{>%s}_{MMM})-BC_\mathrm{%s,MMM}(\delta SM^{>%s}_\mathrm{%s})$ (unitless)' % ('F',pc,'H',pc,'H',pc,'H'))
        ...
        plt.tight_layout()
        plt.savefig('%s/scatter.dpc.devhistcsm.%s.%s.%+05.1f.%+05.1f.%s.%s.dev.pdf' % (odir,varn1,varn2,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
