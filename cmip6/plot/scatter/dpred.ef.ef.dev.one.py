...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            vn2c2 = pickle.load(open('%s/pc%s_%s.%g.%g.%s.dev.pickle' % (idir2c2,varn2,fut,ila,ilo,se), 'rb'))
            ...
            if pc==0:
                ax.set_ylabel(r'$\overline{EF}_\mathrm{%s}-\overline{EF}_\mathrm{%s}$ (unitless)' % ('F','H'))
                ax.set_xlabel(r'$BC(0)_\mathrm{%s}-BC(0)_\mathrm{%s}$ (unitless)' % ('F','H'))
            else:
                ax.set_ylabel(r'$EF^{>%s}_\mathrm{%s}-EF^{>%s}_\mathrm{%s}$ (unitless)' % (pc,'F',pc,'H'))
                ax.set_xlabel(r'$BC(\delta SM^{>%s})_\mathrm{%s}-BC(\delta SM^{>%s})_\mathrm{%s}$ (unitless)' % (pc,'F',pc,'H'))
        ...
        plt.tight_layout()
        plt.savefig('%s/scatter.dpc.%s.%s.%+05.1f.%+05.1f.%s.%s.dev.pdf' % (odir,varn1,varn2,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
