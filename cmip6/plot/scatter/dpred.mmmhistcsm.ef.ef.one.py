...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            vn2c1 = pickle.load(open('%s/pc.mmmhistcsm%s_%s.%g.%g.%s.pickle' % (idir2c1,varn2,his,ila,ilo,se), 'rb'))
            vn2c2 = pickle.load(open('%s/pc.mmmhistcsm%s_%s.%g.%g.%s.pickle' % (idir2c2,varn2,fut,ila,ilo,se), 'rb'))
            ...
            if pc==0:
                ...
                ax.set_xlabel(r'$BC(\overline{SM_{MMM}+\Delta SM})-BC(\overline{SM}_{MMM})$ (unitless)')
            else:
        ...
        plt.tight_layout()
        plt.savefig('%s/scatter.dpc.mmmhistcsm.%s.%s.%+05.1f.%+05.1f.%s.%s.pdf' % (odir,varn1,varn2,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
