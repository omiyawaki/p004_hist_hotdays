    ...
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            if pc==0:
                ...
                ax.set_xlabel(r'$BC_\mathrm{%s,MMM}+\Delta BC(\overline{\delta SM_\mathrm{%s,MMM}})-BC_\mathrm{%s,MMM}(\overline{\delta SM_\mathrm{%s,MMM}})$ (unitless)' % ('H','F','H','H'))
            else:
                ax.set_ylabel(r'$BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s})-BC_\mathrm{%s}(\delta SM^{>%s}_\mathrm{%s})$ (unitless)' % ('F',pc,'F','H',pc,'H'))
                ax.set_xlabel(r'$BC_\mathrm{%s,MMM}+\Delta BC(\delta SM^{>%s}_\mathrm{%s,MMM})-BC_\mathrm{%s,MMM}(\delta SM^{>%s}_\mathrm{%s,MMM})$ (unitless)' % ('H',pc,'F','H',pc,'H'))
        ...
        plt.tight_layout()
        plt.savefig('%s/scatter.dpc.devdbc.%s.%s.%+05.1f.%+05.1f.%s.%s.dev.pdf' % (odir,varn1,varn2,la,lo,pc,se), format='pdf', dpi=300)
        plt.close()
