...
def plot(re):
    ...
    def load_mmm(varn,varnp):
        idir = '/project/amp02/miyawaki/data/p004/cmip6/%s/%s/%s/%s' % (se,fo,md,varn)
        return xr.open_dataarray('%s/pc.%s_%s.%s.nc' % (idir,varn,yr,se))

    tvn1=load_vn(varn,fo,yr,px='pc')
    pvn1=load_mmm(varn1,varnp)
    pct=pvn1['percentile']
    if reverse and (varn1 in ['ti_ev','gflx','hfss','hfls','fa850','fat850','advt850','advtx850','advty850','advm850','advmx850','advmy850','rfa'] or 'ooplh' in varn1):
        pvn1=-pvn1
    if 'wap' in varn1:
        pvn1=pvn1*86400/100 # convert from Pa/s to hPa/d
    ...
    def load_vn(idir0):
        pvne=xr.open_dataarray('%s/pc.%s_%s.%s.nc' % (idir0,varn1,yr,se))
        if varn1=='pr': pvne=86400*pvne
        if 'wap' in varn1: pvne=pvne*86400/100 # convert from Pa/s to hPa/d
        return pvne
    ...
    pvne=[load_vn(idir0) for idir0 in tqdm(idirs)]
    pvne=xr.concat(pvne,'model')
    ...
    nidx=np.isnan(pvn1)
    for imd in range(pvne.shape[0]):
        nidx=np.logical_or(np.isnan(nidx),np.isnan(pvne[imd,...]))
    tvn1[nidx]=np.nan
    pvn1[nidx]=np.nan
    pvne[:,nidx]=np.nan
    ...
    # select region
    ahddp1,kidx=regsla(pvn1,gr,ah)
    tvn1,_=regsla(tvn1,gr,ah)
    ahddpe=eregsla(pvne,gr,ah,kidx)
    # weights
    ...
    clf=ax.pcolormesh(ahmmon,ahmpct,ahddp1,vmin=vmn,vmax=vmx,cmap=cmap(varn1))
    if bstype=='bins' or md=='mmm':
        hatch=plt.fill_between([-0.5,11.5],0,100,hatch='///////',color='none',edgecolor='gray',linewidths=0.3)
        ax.pcolormesh(ahmmon,ahmpct,np.ma.masked_where(al==0,ahddp1),vmin=vmn,vmax=vmx,cmap=cmap(varn1))
