...
for pc in lpc:
    for se in lse:
        ...
        ev=pickle.load(open('%s/ev.dpred.%s.%s.%s.%g.%s.dev.pickle' % (idir,varn,his,fut,pc,se),'rb'))
        ...
        ax.set_ylim([0,1])
        ax.set_xticks(xl)
        ax.set_xticklabels([r'$\Delta BC$',r'$BC_H$',r'$\Delta\delta SM$',r'$\delta SM_H$'])
        if pc==0:
        ...
        plt.tight_layout()
        plt.savefig('%s/ev.dpred.%s.%g.%s.dev.pdf' % (odir,varn,pc,se), format='pdf', dpi=300)
        plt.close()
