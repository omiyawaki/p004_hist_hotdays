...
for pc in lpc:
    for se in lse:
        ...
        ev=pickle.load(open('%s/ev.dpred.%s.act.%s.%s.%g.%g.%g.%s.dev.pickle' % (idir,varn,his,fut,iloc[0],iloc[1],pc,se),'rb'))
        ...
        if pc==0:
            ax.set_xlabel(r'$\Delta(BC(\overline{\delta SM}))$')
            ax.set_ylabel(r'$\Delta \overline{EF}$' '\n' r'Explained Intermodel Variance')
        else:
            ax.set_xlabel(r'$\Delta(BC(\delta SM^{%g}))$'%pc)
            ax.set_ylabel(r'$\Delta EF^{%g}$' '\n' r'Explained Intermodel Variance'%pc)
        ax.set_title(r'%s [%+05.1f,%+05.1f]' % (se.upper(),la,lo))
        plt.tight_layout()
        plt.savefig('%s/ev.act.dpred.%s.%g.%s.dev.pdf' % (odir,varn,pc,se), format='pdf', dpi=300)
        plt.close()
