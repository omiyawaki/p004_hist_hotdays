...
for pc in lpc:
    for se in lse:
        ...
        ev=pickle.load(open('%s/ev.dpred.d%s.act.d%s.%s.%g.%s.pickle' % (idir,varn,his,fut,pc,se),'rb'))
        ...
        ax.set_xlabel(r'$\Delta(BC(SM^{%g}))-\Delta(BC(\overline{SM}))$'%pc)
        ...
        plt.tight_layout()
        plt.savefig('%s/ev.act.dpred.d%s.%g.%s.pdf' % (odir,varn,pc,se), format='pdf', dpi=300)
        plt.close()
