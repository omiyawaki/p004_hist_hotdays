...
for pc in lpc:
    for se in lse:
        ...
        ax.set_ylim([0,1])
        ax.set_xticks(xl)
        ax.set_xticklabels(['All',r'$\Delta BC$',r'$BC_H$',r'$\Delta SM$',r'$SM_H$'])
        if pc==0:
            ax.set_xlabel(r'$\Delta(BC(\overline{SM}))$')
            ax.set_ylabel(r'$\Delta \overline{EF}$' '\n' r'Explained Intermodel Variance')
        else:
            ax.set_xlabel(r'$\Delta(BC(SM^{%g}))$'%pc)
            ax.set_ylabel(r'$\Delta EF^{%g}$' '\n' r'Explained Intermodel Variance'%pc)
        ax.set_title(r'%s [%+05.1f,%+05.1f]' % (se.upper(),la,lo))
        plt.tight_layout()
        plt.savefig('%s/ev.act.dpred.%s.%g.%s.pdf' % (odir,varn,pc,se), format='pdf', dpi=300)
        plt.close()
