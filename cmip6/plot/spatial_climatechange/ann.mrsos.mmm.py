...
cb.set_label(label=r'$\Delta \delta SM^{%02d}$ (kg m$^{-2}$)'%(p),size=16)
fig.savefig('%s/ann.ddp%02d%s.%s.pdf' % (odir,p,varn,fo), format='pdf', dpi=300)

# plot pct warming
fig,ax=plt.subplots(subplot_kw={'projection': ccrs.Robinson(central_longitude=240)},figsize=(5,4),constrained_layout=True)
clf=ax.contourf(mlon, mlat, lldpvn, np.arange(-5,5+0.25,0.25),extend='both', vmax=5, vmin=-5, transform=ccrs.PlateCarree(), cmap='RdBu_r')
ax.coastlines()
ax.set_title(r'%s %s ANN' % (md.upper(),fo.upper()),fontsize=16)
cb=fig.colorbar(clf,location='bottom',aspect=50)
cb.ax.tick_params(labelsize=12)
cb.set_label(label=r'$\Delta SM^{%02d}$ (kg m$^{-2}$)'%(p),size=16)
fig.savefig('%s/ann.dp%02d%s.%s.pdf' % (odir,p,varn,fo), format='pdf', dpi=300)

# plot mean warming
fig,ax=plt.subplots(subplot_kw={'projection': ccrs.Robinson(central_longitude=240)},figsize=(5,4),constrained_layout=True)
clf=ax.contourf(mlon, mlat, lldvn, np.arange(-5,5+0.25,0.25),extend='both', vmax=5, vmin=-5, transform=ccrs.PlateCarree(), cmap='RdBu_r')
ax.coastlines()
ax.set_title(r'%s %s ANN' % (md.upper(),fo.upper()),fontsize=16)
cb=fig.colorbar(clf,location='bottom',aspect=50)
cb.ax.tick_params(labelsize=12)
cb.set_label(label=r'$\Delta \overline{SM}$ (kg m$^{-2}$)',size=16)
