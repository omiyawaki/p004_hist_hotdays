    ...
    for se in lse:
        for fo in lfo:
            ...
            for md in lmd:
                ...
                vlim=1
                clf=ax.contourf(mlon, mlat, 1e3*ddist, np.arange(-vlim,vlim,vlim/10),extend='both', vmax=vlim, vmin=-vlim, transform=ccrs.PlateCarree(), cmap='RdBu_r')
                ax.coastlines()
                ax.set_title(r'%s %s %s' % (se.upper(),md.upper(),fo.upper()))
                cb=plt.colorbar(clf,location='bottom')
                cb.set_label(r'$(q^{>%s}_\mathrm{2\,m}-\overline{q}_\mathrm{2\,m})_\mathrm{%s}-(q^{>%s}_\mathrm{2\,m}-\overline{q}_\mathrm{2\,m})_\mathrm{%s}$ (g kg$^{-1}$)' % (pc,fut,pc,his))
