...
for pc in lpc:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            ax[imd].hist(dhd,bins=be,color=co)
            if pc==95:
                ax[imd].set_ylim([0,40])
            elif pc==99:
                ax[imd].set_ylim([0,15])
            ax[imd].set_xlim([be[0],20])
            ax[imd].set_xticks(be+0.5)
            ax[imd].set_xticklabels(be.astype(int),fontsize=5)
            ax[imd].set_xlabel('Days')
            ax[imd].set_ylabel('Frequency')
            ax[imd].set_title(r'%s'%md)
            plt.tight_layout()
            plt.savefig('%s/hg.cd%s.%g.%s.pdf' % (odir,varn,pc,se), format='pdf', dpi=128)
        plt.close()
