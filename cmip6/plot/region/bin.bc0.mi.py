...
def plot(relb,fo=fo1):
    ...
    ax[i].axvline(csm,color='tab:purple')
...
for relb in lrelb:
    ...
    icsm=np.empty(len(lmd))
    for i,md in enumerate(tqdm(lmd)):
        icsm[i]=plot(relb,fo=fo1)

    ddir= '/project/amp02/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (ose,fo,'mi',varn,'regions')
    if not os.path.exists(ddir):
        os.makedirs(ddir)
    
    icsm=xr.DataArray(icsm,coords={'model':lmd},dims=('model'))
    icsm.to_netcdf('%s/crit.sm.%s.nc'%(ddir,relb),format='NETCDF4')
