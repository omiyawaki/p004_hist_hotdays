...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        _,gr = pickle.load(open('%s/b%s_%s.%s.pickle' % (idir,varn,yr,se), 'rb'))
        ila=iloc[0]
        ilo=iloc[1]
        la=gr['lat'][ila]
        lo=gr['lon'][ilo]
        ...
        [ef,sm]=pickle.load(open(fn,'rb'))

        ax.plot(sm,ef,'-',color=cs[imd],linewidth=1,label=md)
        ax.set_xlim([-30,20])
        ax.set_ylim([0,1.5])
        ...
        plt.savefig('%s/scatter.l.%s.bin.%+05.1f.%+05.1f.%s.pdf' % (odir,varn,la,lo,se), format='pdf', dpi=300)
    ...
    fn = '%s/b%s_%s.%g.%g.%s.dev.pickle' % (idir,varn,yr,iloc[0],iloc[1],se)
    [ef,sm]=pickle.load(open(fn,'rb'))
    ax.plot(sm,ef,'-',color='k',linewidth=2,label='MMM')
    ax.set_xlim([-30,20])
    ax.set_ylim([0,1.5])
    ax.set_title(r'%s [%+05.1f,%+05.1f] (%s)' % (se.upper(),la,lo,yr))
    ax.set_xlabel(r'$\delta SM$ (kg m$^{-2}$)')
