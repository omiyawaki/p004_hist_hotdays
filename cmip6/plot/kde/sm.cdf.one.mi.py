...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        ax[imd].plot(sm,cdf1,label='1980-2000')
        ax[imd].plot(sm,cdf2,label='2080-2100')
        ax[imd].set_title(r'%s' % (md.upper()))
        ax[imd].set_xlim([0,50])
        ax[imd].set_xlabel(r'$SM$ (kg m$^{-2}$)')
        ax[imd].set_ylabel(r'CDF')
        plt.legend()
        plt.tight_layout()
        plt.savefig('%s/cdf.%s.%+05.1f.%+05.1f.%s.pdf' % (odir,varn,la,lo,se), format='pdf', dpi=128)
    plt.close()
