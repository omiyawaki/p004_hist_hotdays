...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        ax.plot(psm,bef,'.',color=cs[imd],markersize=2,label=md)
        ax.set_xlim([psm[0],psm[-1]])
        ax.set_ylim([-30,20])
        ax.set_title(r'%s [%+05.1f,%+05.1f] (%s)' % (se.upper(),la,lo,yr))
        ax.set_xlabel(r'$\delta SM$ Percentile')
        ax.set_ylabel(r'$\delta SM$ (kg m$^{-2}$)')
        plt.tight_layout()
        box = ax.get_position()
        ax.set_position([box.x0, box.y0, box.width * 0.8, box.height])
        plt.legend(loc='center left',bbox_to_anchor=(1, 0.5),prop={'size':7},frameon=False)
        plt.savefig('%s/scatter.%s.pct.%+05.1f.%+05.1f.%s.pdf' % (odir,varn,la,lo,se), format='pdf', dpi=300)
    plt.close()
