...
def make_pct(md,fo,by,re):
    ...
    if md=='CESM2':
        ...
        fsm=-eval_pct('ti_fsm',idx,nidx,by,se,fo,md,re)
    print('Evaluating rsfc pct')
    rsfc=eval_pct('ti_rsfc',idx,nidx,by,se,fo,md,re)
    print('Evaluating hfss pct') # note *-1 so + heats ground
    hfss=-eval_pct('ti_hfss',idx,nidx,by,se,fo,md,re)
    print('Evaluating hfls pct') # *-1
    hfls=-eval_pct('ti_hfls',idx,nidx,by,se,fo,md,re)
    if md=='CESM2':
...
def plot_pct(md,re):
    ...
    ax.plot(x,1e-6*rsfc1,color='gray',label='$R_n$')
    ax.plot(x,1e-6*hfss1,color='tab:orange',label='$SH$')
    ax.plot(x,1e-6*hfls1,color='tab:blue',label='$LH$')
    if md=='CESM2':
        ax.plot(x,1e-6*fsm1,color='purple',label='$F_{melt}$')
    ...
    fig.savefig('%s/pct.%s.%s.%s.%02d.pdf'%(odir,'seb.ti',fo,re,mn),format='pdf',dpi=dpi)
    fig.savefig('%s/pct.%s.%s.%s.%02d.png'%(odir,'seb.ti',fo,re,mn),format='png',dpi=dpi)

    fig,ax=plt.subplots(figsize=(4,3),constrained_layout=True)
    ax.axhline(0,color='gray',linestyle='-',linewidth=0.5)
    ax.axvline(np.interp(273.15,tas1,x),color='k',linestyle='--',linewidth=0.5)
    ax.plot(x,1e-6*(rsfc2-rsfc1),color='gray',label='$\Delta R_n$')
    ax.plot(x,1e-6*(hfss2-hfss1),color='tab:orange',label='$\Delta SH$')
    ax.plot(x,1e-6*(hfls2-hfls1),color='tab:blue',label='$\Delta LH$')
    if md=='CESM2':
        ax.plot(x,1e-6*(fsm2-fsm1),color='purple',label='$\Delta F_{melt}$')
    ax.set_xlabel('Percentile (unitless)')
    ax.set_ylabel('(MJ m$^{-2}$)')
    ax.set_title(tstr(re,mn))
    ax2=ax.twiny()
    ...
    ax2.set_xlabel('T$_{hist}$ (K)')
    ax.legend(frameon=False,fontsize=8)
    fig.savefig('%s/d.pct.%s.%s.%s.%02d.pdf'%(odir,'seb.ti',fo,re,mn),format='pdf',dpi=dpi)
    fig.savefig('%s/d.pct.%s.%s.%s.%02d.png'%(odir,'seb.ti',fo,re,mn),format='png',dpi=dpi)
