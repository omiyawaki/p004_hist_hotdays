...
for m in tqdm(range(12)):
    fig,ax=plt.subplots(nrows=2,ncols=2,subplot_kw={'projection': ccrs.Robinson(central_longitude=240)},figsize=(9,7),constrained_layout=True)
    ax=ax.flatten()
    fig.suptitle(r'%s %s %s' % (md.upper(),fo.upper(),monname(m)),fontsize=16)
    for i in tqdm(range(4)):
        clf=ax[i].contourf(mlon, mlat, lmean1[-i-1][m,...]-lmean1[i][m,...], np.arange(-2.5,2.5+0.25,0.25),extend='both', vmax=2.5, vmin=-2.5, transform=ccrs.PlateCarree(), cmap='RdBu_r')
        ax[i].coastlines()
        ax[i].set_title(r'[%s]$-$[%s]' % (llb[-i-1],llb[i]),fontsize=16)
        fig.savefig('%s/rel.ds.%02d%s.%s.%02d.pdf' % (odir,p,varn,fo,m+1), format='pdf', dpi=300)
    cb=fig.colorbar(clf,ax=ax.ravel().tolist(),location='bottom',aspect=50)
    cb.ax.tick_params(labelsize=16)
    cb.set_label(label=r'$\Delta LH$ anomaly composited on $T^{%02d}$ day  (W m$^{-2}$)'%(p),size=16)
    fig.savefig('%s/rel.ds.%02d%s.%s.%02d.pdf' % (odir,p,varn,fo,m+1), format='pdf', dpi=300)
