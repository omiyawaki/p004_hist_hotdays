...
for pc in lpc:
    for se in lse:
        for imd in tqdm(range(len(lmd))):
            ...
            [c1, gr] = pickle.load(open('%s/c%s_%s.%s.rg.pickle' % (idir0,varn1,yr1,se), 'rb'))
            ila=iloc[0]
            ilo=iloc[1]
            la=gr['lat'][ila]
            lo=gr['lon'][ilo]
            ...
            ll1=ll1[:,ila,ilo]*86400
            ll2=ll2[:,ila,ilo]*86400
            ...
            for vnt in lvnt:
                ...
                ax.set_xlim([-vnt,vnt])
                ax.set_xlabel('Lead-lag (days)')
                ax.set_ylabel(r'$\delta P$ (mm d$^{-1}$)')
                ax.set_title(r'%s %s [%+05.1f,%+05.1f]' % (md,se.upper(),la,lo))
                plt.tight_layout()
