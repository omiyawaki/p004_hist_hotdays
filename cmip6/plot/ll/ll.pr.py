...
ll1=86400*ds[varn] # convert to mm/d
lday=ds['lday']
gpi=ds['gpi']
nl,nm,ng=ll1.shape
...
for m in tqdm(range(12)):
    ...
    ax=ax.flatten()
    fig.suptitle(r'%s %s %s' % (md.upper(),fo1.upper(),monname(m)),fontsize=16)
    for i,ll in enumerate(tqdm(lmean1)):
        clf=ax[i].contourf(mlon, mlat, ll[m,...], np.arange(-10,10+1,1),extend='both', vmax=10, vmin=-10, transform=ccrs.PlateCarree(), cmap='RdBu_r')
        ax[i].coastlines()
        ax[i].set_title(r'%s' % (llb[i]),fontsize=16)
        fig.savefig('%s/ds.%02d%s.%s.%02d.pdf' % (odir1,p,varn,fo1,m+1), format='pdf', dpi=300)
    cb=fig.colorbar(clf,ax=ax.ravel().tolist(),location='bottom',aspect=50)
    cb.ax.tick_params(labelsize=16)
    cb.set_label(label=r'$P$ anomaly composited on $T^{%02d}$ day  (mm d$^{-1}$)'%(p),size=16)
    fig.savefig('%s/ds.%02d%s.%s.%02d.pdf' % (odir1,p,varn,fo1,m+1), format='pdf', dpi=300)
