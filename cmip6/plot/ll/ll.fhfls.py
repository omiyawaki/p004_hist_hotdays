...
# ll data
ds=xr.open_dataset('%s/ll%g.p%g.%s_%s.%s.nc' % (idir2,nt,p,varn,fut,se))
ll1=ds[varn]
lday=ds['lday']
gpi=ds['gpi']
nl,nm,ng=ll1.shape
...
for m in tqdm(range(12)):
    ...
    for i,ll in enumerate(tqdm(lmean1)):
        clf=ax[i].contourf(mlon, mlat, ll[m,...], np.arange(-50,50+5,5),extend='both', vmax=50, vmin=-50, transform=ccrs.PlateCarree(), cmap='RdBu_r')
        ax[i].coastlines()
        ax[i].set_title(r'%s' % (llb[i]),fontsize=16)
        fig.savefig('%s/ds.%02d%s.%s.%02d.pdf' % (odir2,p,varn,fo2,m+1), format='pdf', dpi=300)
    cb=fig.colorbar(clf,ax=ax.ravel().tolist(),location='bottom',aspect=50)
    cb.ax.tick_params(labelsize=16)
    cb.set_label(label=r'$LH$ anomaly composited on $T^{%02d}$ day  (W m$^{-2}$)'%(p),size=16)
    fig.savefig('%s/ds.%02d%s.%s.%02d.pdf' % (odir2,p,varn,fo2,m+1), format='pdf', dpi=300)
