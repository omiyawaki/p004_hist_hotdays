...
def plot(varn,relb,lld0):
    ...
    if varn=='mrsos':
        lld0.data[np.abs(lld0.data)>1e10]=np.nan
    discard=np.nonzero(np.isnan(mask))
    lld=np.nanmean(np.delete(lld0.data,discard,axis=2),axis=2)
    ...
    fig.supylabel('$\Delta$%s anomaly (%s)'%(varnlb(varn),unitlb(varn)),fontsize=16)
    for m in tqdm(range(nm)):
        clf=ax[m].plot(lday,lld[:,m],'-k')
        ax[m].axhline(0,linewidth=0.5,color='k')
        ax[m].set_xlim([-30,30])
        ax[m].set_ylim(setylim(varn,relb))
