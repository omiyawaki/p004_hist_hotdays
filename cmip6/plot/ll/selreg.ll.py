...
def plot(varn,relb,ll1):
    ...
    ll1=np.nanmean(np.delete(ll1.data,np.nonzero(np.isnan(mask)),axis=2),axis=2)
    print(ll1.shape)
    ...
    for m in tqdm(range(nm)):
        ...
        fig.savefig('%s/ds.%02d%s.%s.%s.png' % (odir1,p,varn,fo1,relb), format='png', dpi=300)
    fig.savefig('%s/ds.%02d%s.%s.%s.png' % (odir1,p,varn,fo1,relb), format='png', dpi=300)
