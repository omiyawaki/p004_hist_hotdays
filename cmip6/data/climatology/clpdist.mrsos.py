    ...
    for fo in lfo:
        for se in lse:
            ...
            for imd in tqdm(range(len(lmd))):
                ...
                [ht2m0, gr] = pickle.load(open('%s/c%s_%s.%s.rg.pickle' % (mdir(se,'his','historical',md,varn),varn,his,se), 'rb'))
                [ht2m1, gr] = pickle.load(open('%s/c%s_%s.%s.rg.pickle' % (mdir(se,'fut',fo,md,varn),varn,fut,se), 'rb'))
