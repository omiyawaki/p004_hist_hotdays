...
from etregimes_ef import bestfit_ef,bestfit_hd22
...
def calc_bc(md):
    ...
    print('\n Loading data...')
    ds=xr.open_dataset(get_fn(varnef,md,fo,byr))
    gpi=ds['gpi']
    time=ds['time']
    vn1=ds[varnef]
    ...
    def bcmon(mon,vn1,vn2):
        ...
        for igpi in lgpi:
            ...
            for ibs in tqdm(range(nbs)):
                bvn=resample(nvn)
                try:
                    f1,f2=bestfit_hd22(bvn[:,1],bvn[:,0])
                    bc[0].append(f2['line'])
                    csm[igpi,ibs]=f2['xc']
                except:
                    bc[0].append(None)
    ...
    # save bc
    if 'gwl' in byr:
        oname='%s/%s_bs.%s.%s.%s.pickle' % (odir,varn,byr,se,lgpi[0])
    else:
        oname='%s/%s_bs.%g-%g.%s.%s.pickle' % (odir,varn,byr[0],byr[1],se,lgpi[0])
    ...
    csm=csm.rename('csm')
    if 'gwl' in byr:
        oname='%s/%s_bs.%s.%s.%s.nc' % (odir,varn,byr,se,lgpi[0])
    else:
        oname='%s/%s_bs.%g-%g.%s.%s.nc' % (odir,varn,byr[0],byr[1],se,lgpi[0])
