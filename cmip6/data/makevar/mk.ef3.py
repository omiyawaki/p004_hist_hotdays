...
def calc_ef3(md):
    ...
    chk=0
    idir0='/project/amp02/miyawaki/data/share/cmip6/%s/%s/%s/%s/%s/%s' % (fo,freq,'rf',md,ens,grd)
    idir='/project/mojave/cmip6/%s/%s/%s/%s/%s/%s' % (fo,freq,'hfls',md,ens,grd)
    for _,_,files in os.walk(idir0):
        for fn in files:
            print(fn)
            ofn='%s/%s'%(odir,fn.replace('rf',varn))
            try:
                xr.load_dataset(ofn)
                ...
                rf=ds['rf']
                fn1='%s/%s'%(idir,fn.replace('rf','hfls',1))
                ds = xr.open_dataset(fn1)
                hfls=ds['hfls']
                # compute evaporative fraction (EF)
                ef3=rf.copy()
                ef3.data=hfls.data/rf.data
                ef3=ef3.rename(varn)
                ef3.to_netcdf(ofn)

if __name__ == '__main__':
    with ProgressBar():
        tasks=[dask.delayed(calc_ef3)(md) for md in lmd]
