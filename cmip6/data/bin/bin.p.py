...
varn='tas'
vnpre='dp' # prefix of variable to compute
bnpre='ddp' # prefix of variable to bin
if bnpre=='ddp':
    lvn=np.arange(-5,5+0.1,0.1) # bins for relative warming of hotdays
else:
    lvn=np.linspace(-5,20,101) # bins for warming of xth pct day
...
def binp(flag):
    ...
    b=np.empty([ivn.shape[0],ivn.shape[2],len(lvn)])
    for imd in tqdm(range(ivn.shape[0])):
        for ip in range(ivn.shape[2]):
            ...
            sw=sw[~np.isnan(sw)]
            dg=np.digitize(sbvn,lvn)
            b[imd,ip,:]=[np.nansum(sw[dg==i]*svn[dg==i])/np.nansum(sw[dg==i]) for i in range(len(lvn))]

    odir = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,cl,fo,'mi',varn)

    oname='%s/b.%s.%s%s_%s_%s.%s' % (odir,bnpre,vnpre,varn,his,fut,se)
    if flag['landonly']:
        oname='%s.land'%oname
    if flag['troponly']:
        oname='%s.trop'%oname

    pickle.dump([b,gr['pct'],lvn], open('%s.pickle'%oname, 'wb'), protocol=5)	
