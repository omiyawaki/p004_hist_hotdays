...
arr1=np.linspace(250,350,1000)
arr2=np.linspace(0,100,1000)
msm,mef=np.mgrid[0:100:1000j,250:350:1000j]
abm=np.vstack([msm.ravel(),mef.ravel()])
varn='%s+%s'%(varn1,varn2)
varna='%s+%s'%(varn2,varn1)
...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        odir='/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,cl,fo,md,varna)

        fn = '%s/k%s_%s.%g.%g.%s.pickle' % (idir,varn,yr,iloc[0],iloc[1],se)
        kde = pickle.load(open(fn,'rb'))
        pdf=np.reshape(kde(abm).T,msm.shape)
        sm=np.trapz(pdf*msm,x=arr2,axis=0)/np.trapz(pdf,x=arr2,axis=0)
        stdev=np.sqrt(np.trapz(pdf*(msm-sm[:,None])**2,x=arr2,axis=0)/np.trapz(pdf,x=arr2,axis=0))
        ...
        ila=iloc[0]
        ilo=iloc[1]
        pickle.dump([sm,arr1,stdev], open('%s/%s_%s.%g.%g.%s.pickle' % (odir,varna,yr,ila,ilo,se), 'wb'), protocol=5)	
