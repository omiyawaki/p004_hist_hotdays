    ...
    for fo in lfo:
        for cl in lcl:
            ...
            for imd in tqdm(range(len(lmd))):
                ...
                cdmrsos = np.empty([len(pc), gr['lat'].size, gr['lon'].size])
                ...
                for ln in tqdm(range(gr['lon'].size)):
                    for la in range(gr['lat'].size):
                        ...
                        lt95 = ht2m95[la,ln]
                        lt99 = ht2m99[la,ln]
                        cdmrsos[0,la,ln]=np.nanmean(lv)
                        cdmrsos[1,la,ln]=np.nanmean(lv[np.where(lt>lt95)])
                        cdmrsos[2,la,ln]=np.nanmean(lv[np.where(lt>lt99)])

                pickle.dump([cdmrsos, gr], open('%s/p%s_%g-%g.%s.pickle' % (idir,varn2,byr[0],byr[1],se), 'wb'), protocol=5)	
