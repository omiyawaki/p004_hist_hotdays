...
def calc_lm(md):
    ...
    for varn in lvn:
        ...
        # take seasonal mean
        lat=vn['lat']
        if se in ['winter','summer']:
            vnj=vn.sel(time=vn['time.season']=='JJA').mean('time')
            vnd=vn.sel(time=vn['time.season']=='DJF').mean('time')

        if se=='winter':
            vn=vnj.copy()
            vn.data[lat>0]=vnd.data[lat>0]
        elif se=='summer':
            vn=vnd.copy()
            vn.data[lat>0]=vnj.data[lat>0]
        elif se=='ond+amj':
            vno=vn.sel(time=vn['time.month'].isin([10,11,12])).mean('time')
            vna=vn.sel(time=vn['time.month'].isin([4,5,6])).mean('time')
            vn=vna.copy()
            vn.data[lat>0]=vno.data[lat>0]
        ...
        vn=vn.rename(varn)
        if 'gwl' in byr:
            vn.to_netcdf('%s/m.%s_%s.%s.nc' % (odir,varn,byr,se),format='NETCDF4')
        else:
            vn.to_netcdf('%s/m.%s_%g-%g.%s.nc' % (odir,varn,byr[0],byr[1],se),format='NETCDF4')
