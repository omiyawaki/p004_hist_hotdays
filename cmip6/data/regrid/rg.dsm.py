...
# lnt=[30] # number of days to accumulate data
        ...
        for se in lse:
            ...
            for imd in tqdm(range(len(lmd))):
                ...
                if md=='IITM-ESM':
                    sm=1e3*sm
                ds=None
                # take difference in sm from nt days prior
                dsm=sm-np.roll(sm,nt,axis=0)
                # express in mm/d
                dsm=dsm/nt

                # select data within time of interest
                dsm=dsm.sel(time=dsm['time.year']>=byr[0])
                dsm=dsm.sel(time=dsm['time.year']<=byr[1])

                # select seasonal data if applicable
                if se != 'ann':
                    dsm=dsm.sel(time=dsm['time.season']==se.upper())

                if md!='CESM2':
                    # path to weight file
                    wf='%s/wgt.cmip6.%s.%s.cesm2.nc'%(rgdir,md,ty)
                    # build regridder with existing weights
                    rgd = xe.Regridder(dsm,ogr, 'bilinear', periodic=True, reuse_weights=True, filename=wf)
                    # regrid
                    dsm=rgd(dsm)

                dsm=dsm.rename(varn)
                dsm.to_netcdf('%s/cl%s%03d_%g-%g.%s.nc' % (odir,varn,nt,byr[0],byr[1],se))
