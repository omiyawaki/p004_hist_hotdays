...
# lvn=['tas','hfls','hfss','rsfc','gflx'] # input1
lvn=['ooplh','ooplh_fixbc','ooplh_fixmsm','ooplh_rddsm'] # input1
ty='2d'
checkexist=False
doy=False
...
def calc_mvn(md):
    ...
    for varn in lvn:
        ...
        if doy:
            if 'gwl' in byr:
                oname='%s/md.doy.%s_%s.%s.nc'%(odir,varn,byr,se)
            else:
                oname='%s/md.doy.%s_%g-%g.%s.nc'%(odir,varn,byr[0],byr[1],se)
        else:
            if 'gwl' in byr:
                oname='%s/md.%s_%s.%s.nc'%(odir,varn,byr,se)
            else:
                oname='%s/md.%s_%g-%g.%s.nc'%(odir,varn,byr[0],byr[1],se)
        ...
        if doy:
            ...
            mvn=vn.groupby('time.dayofyear').median('time')
            ...
            mvn=vn.groupby('time.month').median('time')
            print('\n Done.')
