...
def calc_p(md):
    ...
    for varn in lvn:
        ...
        if checkexist:
            if 'gwl' in byr:
                if os.path.isfile('%s/stdp%s_%s.%s.nc' % (odir,varn,byr,se)):
                    print('Output file already exists, skipping...')
                    continue
            else:
                if os.path.isfile('%s/stdp%s_%g-%g.%s.nc' % (odir,varn,byr[0],byr[1],se)):
                    print('Output file already exists, skipping...')
                    continue
        ...
        svnhd.data=tpd.groupby('time.dayofyear').std('time',skipna=True).data
        ...
        if 'gwl' in byr:
            svnhd.to_netcdf('%s/stdp%s_%s.%s.nc' % (odir,varn,byr,se),format='NETCDF4')
        else:
            svnhd.to_netcdf('%s/stdp%s_%g-%g.%s.nc' % (odir,varn,byr[0],byr[1],se),format='NETCDF4')
