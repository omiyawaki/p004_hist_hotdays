...
def calc_lm(md):
    ...
    for varn in lvn:
        ...
        if 'gwl' in byr:
            ifn='%s/doy.%s_%s.%s.nc' % (odir,varn,byr,se)
            ofn='%s/lm.%s_%s.%s.nc' % (odir,varn,byr,se)
        else:
            ifn='%s/doy.%s_%g-%g.%s.nc' % (odir,varn,byr[0],byr[1],se)
        ...
        ds=xr.Dataset(
                    data_vars={'dx':(['dayofyear','gpi'],dx),'dy':(['dayofyear','gpi'],dy)},
                    coords={'dayofyear':doy,'gpi':np.arange(ngp)}
                    )
