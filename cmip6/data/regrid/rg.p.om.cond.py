...
def calc_p(md):
    ...
    for varn in lvn:
        ...
        if doy:
            if 'gwl' in byr:
                oname='%s/pc.om.doy.%s_%s.%s.nc' % (odir,varn,byr,se)
            else:
                oname='%s/pc.om.doy.%s_%g-%g.%s.nc' % (odir,varn,byr[0],byr[1],se)
        else:
            if 'gwl' in byr:
        ...
        if 'gwl' in byr:
            ds=xr.open_dataset('%s/p.om.%s%03d_%s.%s.nc' % (tdir,'tas',nt,byr,se))
        else:
            ds=xr.open_dataset('%s/p.om.%s%03d_%g-%g.%s.nc' % (tdir,'tas',nt,byr[0],byr[1],se))
        pct=ds['percentile']
        try:
            pvn=ds['tas']
        except:
