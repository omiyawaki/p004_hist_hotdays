...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        [ihfls,iarr2] = pickle.load(open(fn,'rb'))

        if imd==0:
            ehfls=np.empty([len(lmd),len(ihfls)])

        ehfls[imd,:]=ihfls

        hfls=np.nanmean(ehfls,axis=0)

    pickle.dump([hfls,iarr2,ehfls], open('%s/b%s_%s.%g.%g.%s.dev.pickle' % (odir,varn,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
