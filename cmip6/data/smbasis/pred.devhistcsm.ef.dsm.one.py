...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        if cl=='his':
            ...
            idir0 = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,'his','historical',md,varn2)
            [vn0, _] = pickle.load(open('%s/c%s_%s.%s.rg.pickle' % (idir0,varn2,'1980-2000',se), 'rb'))
            l0=vn0[:,iloc[0],iloc[1]]
            idir1 = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,'his','historical','mmm',varn2)
            idir2 = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,cl,fo,'mmm',varn2)
            l1 = pickle.load(open('%s/pcm%s_%s.%g.%g.%s.dev.pickle' % (idir1,varn2,'1980-2000',iloc[0],iloc[1],se), 'rb'))
            l2 = pickle.load(open('%s/pcm%s_%s.%g.%g.%s.dev.pickle' % (idir2,varn2,yr,iloc[0],iloc[1],se), 'rb'))
            l=l0+l2-l1
            # l[0]=0
        ...
        pickle.dump(pef, open('%s/pc.devhistcsm%s_%s.%g.%g.%s.dev.pickle' % (odir,varn1,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
