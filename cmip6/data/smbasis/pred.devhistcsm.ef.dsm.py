...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        if cl=='his':
            ...
            [l, _] = pickle.load(open('%s/c%s_%s.%s.rg.pickle' % (idir,varn2,yr,se), 'rb'))
        elif cl=='fut':
            idir0 = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,'his','historical',md,varn2)
            [l0, _] = pickle.load(open('%s/c%s_%s.%s.rg.pickle' % (idir0,varn2,'1980-2000',se), 'rb'))
            ...
            l1 = pickle.load(open('%s/pcm%s_%s.%s.dev.pickle' % (idir1,varn2,'1980-2000',se), 'rb'))
            l2 = pickle.load(open('%s/pcm%s_%s.%s.dev.pickle' % (idir2,varn2,yr,se), 'rb'))
            l=l0+l2-l1
        ...
        pickle.dump(pef, open('%s/pc.devhistcsm%s_%s.%s.dev.pickle' % (odir,varn1,yr,se), 'wb'), protocol=5)	
