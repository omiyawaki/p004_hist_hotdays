...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        l=vn[:,iloc[0],iloc[1]]
        [dsm, _] = pickle.load(open('%s/dpc%s_%s.%s.%g.%g.%s.pickle' % (idir2,varn2,yr0,yr,iloc[0],iloc[1],se), 'rb'))
        l=l+dsm
        idx=np.argmin(np.abs(np.tile(arr2,[3,1])-l[:,None]),axis=1)
        pef=ef[idx]

        pickle.dump(pef, open('%s/pc.mmmcsm%s_%s.%g.%g.%s.pickle' % (odir,varn1,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
