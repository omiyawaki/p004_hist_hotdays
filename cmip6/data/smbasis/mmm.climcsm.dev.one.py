...
for se in lse:
    ...
    pickle.dump(sm1, open('%s/pcm%s_%s.%g.%g.%s.dev.pickle' % (odir1,varn,yr1,iloc[0],iloc[1],se), 'wb'), protocol=5)	
    pickle.dump(sm2, open('%s/pcm%s_%s.%g.%g.%s.dev.pickle' % (odir2,varn,yr2,iloc[0],iloc[1],se), 'wb'), protocol=5)	
