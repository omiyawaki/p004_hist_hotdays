...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        if cl=='his':
            ...
            fn0 = '%s/%s_%s.%g.%g.%s.pickle' % (idir0,varn,'1980-2000',iloc[0],iloc[1],se)
            [ef0,arr2,_,_,_] = pickle.load(open(fn0,'rb'))
            # load change in bc
            ...
            [ef1,_,_]=pickle.load(open('%s/%s_%s.%g.%g.%s.pickle' % (idir1,varn,'1980-2000',iloc[0],iloc[1],se), 'rb'))
            [ef2,_,_]=pickle.load(open('%s/%s_%s.%g.%g.%s.pickle' % (idir2,varn,yr,iloc[0],iloc[1],se), 'rb'))
            delef=ef2-ef1
            ef=ef0+delef

        # >95th and mean sm data
        idir = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,cl,fo,'mmm',varn2)
        l = pickle.load(open('%s/pcm%s_%s.%g.%g.%s.pickle' % (idir,varn2,yr,iloc[0],iloc[1],se), 'rb'))
        idx=np.argmin(np.abs(np.tile(arr2,[3,1])-l[:,None]),axis=1)
        pef=ef[idx]

        pickle.dump(pef, open('%s/pc.devdbc%s_%s.%g.%g.%s.pickle' % (odir,varn1,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
