...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        idir0='/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,cl0,fo0,md,varn)
        ...
        fn = '%s/%s_%s.%g.%g.%s.pickle' % (idir0,varn,yr0,iloc[0],iloc[1],se)
        ...
        pickle.dump(pef, open('%s/pc.hisbc%s_%s.%g.%g.%s.dev.pickle' % (odir,varn1,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
