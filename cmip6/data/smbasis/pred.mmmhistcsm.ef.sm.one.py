...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        l = pickle.load(open('%s/pc%s_%s.%g.%g.%s.pickle' % (idir,varn2,yr,iloc[0],iloc[1],se), 'rb'))
        if fo=='ssp370':
            l=l[imd,:]
        idx=np.argmin(np.abs(np.tile(arr2,[3,1])-l[:,None]),axis=1)
        pef=ef[idx]

        pickle.dump(pef, open('%s/pc.mmmhistcsm%s_%s.%g.%g.%s.pickle' % (odir,varn1,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
