...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        fn1 = '%s/b%s_%s.%s.dev.pickle' % (idir1,varn,yr1,se)
        fn2 = '%s/b%s_%s.%s.dev.pickle' % (idir2,varn,yr2,se)
        [ief1,iarr2] = pickle.load(open(fn1,'rb'))
        [ief2,_] = pickle.load(open(fn2,'rb'))
    ...
    pickle.dump([ef,iarr2], open('%s/db%s_%s.%s.%s.dev.pickle' % (odir,varn,yr1,yr2,se), 'wb'), protocol=5)	
