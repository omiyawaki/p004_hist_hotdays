...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        # interpolate
        fint=interp1d(bsm,bef,kind='linear',bounds_error=False,fill_value=np.nan)
        ef=fint(vsm)

        # smooth data with running mean
        ef=generic_filter(ef,np.nanmean,size=n_smooth,mode='constant',cval=np.nan) 
        ...
        ila=iloc[0]
        ilo=iloc[1]
        pickle.dump([ef,vsm], open('%s/b%s_%s.%g.%g.%s.dev.pickle' % (odir,varn,yr,ila,ilo,se), 'wb'), protocol=5)	
