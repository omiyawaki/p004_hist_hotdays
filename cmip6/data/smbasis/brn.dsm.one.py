...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        brsfc,_ = pickle.load(open(fn,'rb'))
        brsfc=brsfc[:,iloc[0],iloc[1]]
        # load sm
        ...
        # interpolate
        fint=interp1d(bsm,brsfc,kind='linear',bounds_error=False,fill_value=np.nan)
        rsfc=fint(vsm)

        # smooth data with running mean
        rsfc=generic_filter(rsfc,np.nanmean,size=n_smooth,mode='constant',cval=np.nan) 
        ...
        ila=iloc[0]
        ilo=iloc[1]
        pickle.dump([rsfc,vsm], open('%s/b%s_%s.%g.%g.%s.dev.pickle' % (odir,varn,yr,ila,ilo,se), 'wb'), protocol=5)	
