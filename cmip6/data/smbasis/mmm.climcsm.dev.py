...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        if imd==0:
            ...
            esm2=np.empty([len(lmd),vn1.shape[0],vn1.shape[1],vn1.shape[2]])
    ...
    pickle.dump(sm1, open('%s/pcm%s_%s.%s.dev.pickle' % (odir1,varn,yr1,se), 'wb'), protocol=5)	
    pickle.dump(sm2, open('%s/pcm%s_%s.%s.dev.pickle' % (odir2,varn,yr2,se), 'wb'), protocol=5)	
