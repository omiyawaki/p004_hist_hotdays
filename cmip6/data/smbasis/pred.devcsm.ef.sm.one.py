...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        if cl=='his':
            ...
            l0 = pickle.load(open('%s/pcm%s_%s.%g.%g.%s.pickle' % (idir0,varn2,'1980-2000',iloc[0],iloc[1],se), 'rb'))
        ...
        pickle.dump(pef, open('%s/pc.devcsm%s_%s.%g.%g.%s.pickle' % (odir,varn1,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
