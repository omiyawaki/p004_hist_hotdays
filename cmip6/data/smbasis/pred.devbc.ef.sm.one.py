...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        if cl=='his':
            ...
            [delef,_]=pickle.load(open('%s/d%s_%s.%s.%g.%g.%s.pickle' % (idir1,varn,'1980-2000',yr,iloc[0],iloc[1],se), 'rb'))
            ef=ef0+delef
        ...
        pickle.dump(pef, open('%s/pc.devbc%s_%s.%g.%g.%s.pickle' % (odir,varn1,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
