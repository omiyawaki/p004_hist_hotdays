...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        # interpolate
        ef=np.empty([len(vsm),bef.shape[1],bef.shape[2]])
        for ila in range(bef.shape[1]):
            for ilo in range(bef.shape[2]):
                fint=interp1d(bsm[:,ila,ilo],bef[:,ila,ilo],kind='linear',bounds_error=False,fill_value=np.nan)
                lef=fint(vsm)

                # smooth data with running mean
                ef[:,ila,ilo]=generic_filter(lef,np.nanmean,size=n_smooth,mode='constant',cval=np.nan) 

        if not os.path.exists(odir):
            os.makedirs(odir)

        pickle.dump([ef,vsm], open('%s/b%s_%s.%s.dev.pickle' % (odir,varn,yr,se), 'wb'), protocol=5)	
