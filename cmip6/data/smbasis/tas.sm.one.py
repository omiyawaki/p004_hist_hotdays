...
arr1=np.linspace(100,400,1000)
arr2=np.linspace(0,100,1000)
msm,mef=np.mgrid[0:100:1000j,100:400:1000j]
...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        ila=iloc[0]
        ilo=iloc[1]
        pickle.dump([ef,arr2,stdev], open('%s/%s_%s.%g.%g.%s.pickle' % (odir,varn,yr,ila,ilo,se), 'wb'), protocol=5)	
