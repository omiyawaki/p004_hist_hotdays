...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        if cl=='his':
            ...
            l1 = pickle.load(open('%s/pcm%s_%s.%g.%g.%s.pickle' % (idir1,varn2,'1980-2000',iloc[0],iloc[1],se), 'rb'))
            l2 = pickle.load(open('%s/pcm%s_%s.%g.%g.%s.pickle' % (idir2,varn2,yr,iloc[0],iloc[1],se), 'rb'))
            l=l0+l2-l1

        idx=np.argmin(np.abs(np.tile(arr2,[3,1])-l[:,None]),axis=1)
        pef=ef[idx]

        pickle.dump(pef, open('%s/pc.devhistcsm%s_%s.%g.%g.%s.pickle' % (odir,varn1,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
