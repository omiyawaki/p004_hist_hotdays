...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        idir1 = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,cl0,fo0,md,varn2)
        idir2 = '/project/amp/miyawaki/data/p004/cmip6/%s/%s/%s/%s/%s' % (se,'fut-his','ssp370','mmm',varn2)
        [vn, _] = pickle.load(open('%s/c%s_%s.%s.rg.pickle' % (idir1,varn2,yr0,se), 'rb'))
        l=vn[:,iloc[0],iloc[1]]
        [dsm, _] = pickle.load(open('%s/dpc%s_%s.%s.%g.%g.%s.dev.pickle' % (idir2,varn2,yr0,yr,iloc[0],iloc[1],se), 'rb'))
        l=l+dsm
        idx=np.argmin(np.abs(np.tile(arr2,[3,1])-l[:,None]),axis=1)
        pef=ef[idx]

        pickle.dump(pef, open('%s/pc.mmmcsm%s_%s.%g.%g.%s.dev.pickle' % (odir,varn1,yr,iloc[0],iloc[1],se), 'wb'), protocol=5)	
