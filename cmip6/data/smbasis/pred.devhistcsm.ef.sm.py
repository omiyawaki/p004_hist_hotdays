...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        if cl=='his':
            ...
            l1 = pickle.load(open('%s/pcm%s_%s.%s.pickle' % (idir1,varn2,'1980-2000',se), 'rb'))
            l2 = pickle.load(open('%s/pcm%s_%s.%s.pickle' % (idir2,varn2,yr,se), 'rb'))
            l=l0+l2-l1
        ...
        pickle.dump(pef, open('%s/pc.devhistcsm%s_%s.%s.pickle' % (odir,varn1,yr,se), 'wb'), protocol=5)	
