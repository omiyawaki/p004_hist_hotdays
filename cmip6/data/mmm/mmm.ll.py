...
def calc_mmm(varn):
    for i,md in enumerate(tqdm(lmd)):
        ...
        pvn1=xr.open_dataarray('%s/ll%02d.p%02d.%s_%s.%s.nc' % (idir1,nt,p,varn,his,se),decode_times=False)
        pvn2=xr.open_dataarray('%s/ll%02d.p%02d.%s_%s.%s.nc' % (idir2,nt,p,varn,fut,se),decode_times=False)
        print(pvn1[30,0,8000:8020].data)
    ...
    mpvn1.to_netcdf('%s/ll%02d.p%02d.%s_%s.%s.nc' % (odir1,nt,p,varn,his,se))
    mpvn2.to_netcdf('%s/ll%02d.p%02d.%s_%s.%s.nc' % (odir2,nt,p,varn,fut,se))
    spvn1.to_netcdf('%s/std.ll%02d.p%02d.%s_%s.%s.nc' % (odir1,nt,p,varn,his,se))
    spvn2.to_netcdf('%s/std.ll%02d.p%02d.%s_%s.%s.nc' % (odir2,nt,p,varn,fut,se))

    mdvn.to_netcdf('%s/ll%02d.d%02d.%s_%s_%s.%s.nc' % (odir,nt,p,varn,his,fut,se))
    sdvn.to_netcdf('%s/std.ll%02d.d%02d.%s_%s_%s.%s.nc' % (odir,nt,p,varn,his,fut,se))
