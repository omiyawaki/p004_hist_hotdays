...
p=95 # percentile
lvn=['hfss','hfls','rsds','rsus','rlds','rlus']
...
def calc_mmm(varn):
    for i,md in enumerate(tqdm(lmd)):
        ...
        # prc temp
        ds1=xr.open_dataset('%s/dsp%s_%s.%s.%s.nc' % (idir1,varn,his,p,se))
        try:
            pvn1=ds1[varn]
        except:
            pvn1=ds1['__xarray_dataarray_variable__']
        pvn1=pvn1.groupby('time.month').mean('time') # monthly means
        ds2=xr.open_dataset('%s/dsp%s_%s.%s.%s.nc' % (idir2,varn,fut,p,se))
        try:
            pvn2=ds2[varn]
        except:
    ...
    pickle.dump([mpvn1,spvn1,gr], open('%s/dsp%s_%s.%s.%s.pickle' % (odir1,varn,his,p,se), 'wb'), protocol=5)	
    pickle.dump([mpvn2,spvn2,gr], open('%s/dsp%s_%s.%s.%s.pickle' % (odir2,varn,fut,p,se), 'wb'), protocol=5)	
    pickle.dump([mdpvn,sdpvn,gr], open('%s/ddsp%s_%s_%s.%s.%s.pickle' % (odir,varn,his,fut,p,se), 'wb'), protocol=5)	
    ...
    pickle.dump(ipvn1, open('%s/dsp%s_%s.%s.%s.pickle' % (odir1,varn,his,p,se), 'wb'), protocol=5)	
    pickle.dump(ipvn2, open('%s/dsp%s_%s.%s.%s.pickle' % (odir2,varn,fut,p,se), 'wb'), protocol=5)	
    pickle.dump(idpvn, open('%s/ddsp%s_%s_%s.%s.%s.pickle' % (odir,varn,his,fut,p,se), 'wb'), protocol=5)	
