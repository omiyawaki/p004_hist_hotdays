...
def calc_mmm(varn):
    ...
    for i,md in enumerate(tqdm(lmd)):
        ...
        ds1=xr.open_dataset('%s/pc.om.%s_%s.%s.nc' % (idir1,varn0,his,se))
        try:
            pvn1=ds1[varn0]
        except:
            pvn1=ds1['__xarray_dataarray_variable__']
        ds2=xr.open_dataset('%s/pc.om.%s_%s.%s.nc' % (idir2,varn,fut,se))
        try:
            pvn2=ds2[varn]
        except:
            try:
        ...
        # mean
        ds1=xr.open_dataset('%s/m.om.%s_%s.%s.nc' % (idir1,varn0,his,se))
        try:
            mvn1=ds1[varn0]
        except:
            mvn1=ds1['__xarray_dataarray_variable__']
        ds2=xr.open_dataset('%s/m.om.%s_%s.%s.nc' % (idir2,varn,fut,se))
        try:
            mvn2=ds2[varn0]
        except:
            try:
    ...
    mmvn1.to_netcdf('%s/m.om.%s_%s.%s.nc' % (odir1,varn,his,se))
    mmvn2.to_netcdf('%s/m.om.%s_%s.%s.nc' % (odir2,varn,fut,se))
    smvn1.to_netcdf('%s/std.m.om.%s_%s.%s.nc' % (odir1,varn,his,se))
    smvn2.to_netcdf('%s/std.m.om.%s_%s.%s.nc' % (odir2,varn,fut,se))
    ...
    mdvn.to_netcdf('%s/d.om.%s_%s_%s.%s.nc' % (odir,varn,his,fut,se))
    sdvn.to_netcdf('%s/std.d.om.%s_%s_%s.%s.nc' % (odir,varn,his,fut,se))

    mdpvn.to_netcdf('%s/dpc.om.%s_%s_%s.%s.nc' % (odir,varn,his,fut,se))
    sdpvn.to_netcdf('%s/std.dpc.om.%s_%s_%s.%s.nc' % (odir,varn,his,fut,se))

    mddpvn.to_netcdf('%s/ddpc.om.%s_%s_%s.%s.nc' % (odir,varn,his,fut,se))
    sddpvn.to_netcdf('%s/std.ddpc.om.%s_%s_%s.%s.nc' % (odir,varn,his,fut,se))
