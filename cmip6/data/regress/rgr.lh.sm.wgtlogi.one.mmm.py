...
for se in lse:
    ...
    for imd in tqdm(range(len(lmd))):
        ...
        try:
            popt, pcov = curve_fit(logifunc, l2, l1, p0=[a,x0,1],sigma=pdf**(1/3),absolute_sigma=True)
        except:
            try:
                popt, pcov = curve_fit(logifunc, l2, l1, p0=[a,x0,-1],sigma=pdf**(-1),absolute_sigma=True)
            except:
