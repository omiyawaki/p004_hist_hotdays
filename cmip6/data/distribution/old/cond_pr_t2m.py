    ...
    for fo in lfo:
        for cl in lcl:
            ...
            for imd in tqdm(range(len(lmd))):
                ...
                pr=pr.sel(time=pr['time.year']>=byr[0])
                pr=pr.sel(time=pr['time.year']<=byr[1])
                ...
                cpr = np.empty([len(pc), gr['lat'].size, gr['lon'].size])
                ...
                for ln in tqdm(range(gr['lon'].size)):
                    for la in range(gr['lat'].size):
                        ...
                        lt95 = ht2m95[la,ln]
                        lt99 = ht2m99[la,ln]
                        cpr[0,la,ln]=np.nanmean(lv)
                        cpr[1,la,ln]=np.nanmean(lv[np.where(lt>lt95)])
                        cpr[2,la,ln]=np.nanmean(lv[np.where(lt>lt99)])

                pickle.dump([cpr, gr], open('%s/c%s_%g-%g.%s.pickle' % (odir,varn,byr[0],byr[1],se), 'wb'), protocol=5)	
