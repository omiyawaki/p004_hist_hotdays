    ...
    for fo in lfo:
        for cl in lcl:
            ...
            for imd in tqdm(range(len(lmd))):
                ...
                csfcwind = np.empty([len(pc), gr['lat'].size, gr['lon'].size])
                ...
                for ln in tqdm(range(gr['lon'].size)):
                    for la in range(gr['lat'].size):
                        ...
                        lt95 = ht2m95[la,ln]
                        lt99 = ht2m99[la,ln]
                        csfcwind[0,la,ln]=np.nanmean(lv)
                        csfcwind[1,la,ln]=np.nanmean(lv[np.where(lt>lt95)])
                        csfcwind[2,la,ln]=np.nanmean(lv[np.where(lt>lt99)])

                pickle.dump([csfcwind, gr], open('%s/c%s_%s.%s.rg.pickle' % (odir,varn,byr,se), 'wb'), protocol=5)	
