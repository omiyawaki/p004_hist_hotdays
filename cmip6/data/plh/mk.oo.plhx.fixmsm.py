...
def calc_plh(depth):
    ...
    print('\n Loading data...')
    ds=xr.open_dataset(get_fnh(vmrs,md)) # load historical hot sm
    pct=ds['percentile']
    gpi=ds['gpi']
    sm=ds[vmrs]
    if only95:
    ...
    msmh=xr.open_dataarray(get_mfnh(vmrs,md))
    msm=xr.open_dataarray(get_mfn(vmrs,md))
