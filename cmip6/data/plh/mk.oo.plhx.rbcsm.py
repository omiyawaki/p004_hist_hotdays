...
def calc_plh(depth):
    ...
    lhf=xr.open_dataarray(get_fn('ooplh%g'%depth,fo,byr,md))
    lhfbc=xr.open_dataarray(get_fn('ooplh_fixbc%g'%depth,fo,byr,md))
    lhfsm=xr.open_dataarray(get_fn('ooplh_dbc%g'%depth,fo,byr,md))
    print('\n Done.')

    print('\n Computing residual lh...')
    plh=lhf.copy()
    plh.data=2*lh0.data+lhf.data-lhfbc.data-lhfsm.data
