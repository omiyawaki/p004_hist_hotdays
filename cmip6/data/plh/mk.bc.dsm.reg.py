...
def calc_bc(reg):
    ...
    # save bc
    if 'gwl' in byr:
        oname='%s/%s.%s.%s.%s.pickle' % (odir,varn,byr,se,reg)
    else:
        oname='%s/%s.%g-%g.%s.%s.pickle' % (odir,varn,byr[0],byr[1],se,reg)
    pickle.dump(bc,open(oname,'wb'),protocol=5)

    # save csm
    if 'gwl' in byr:
        oname='%s/%s.%s.%s.%s.pickle' % (odir,'csm',byr,se,reg)
    else:
        oname='%s/%s.%g-%g.%s.%s.pickle' % (odir,'csm',byr[0],byr[1],se,reg)
    pickle.dump(csm,open(oname,'wb'))

    # save mt
    if 'gwl' in byr:
        oname='%s/%s.%s.%s.%s.pickle' % (odir,'mt',byr,se,reg)
    else:
        oname='%s/%s.%g-%g.%s.%s.pickle' % (odir,'mt',byr[0],byr[1],se,reg)
    pickle.dump(mt,open(oname,'wb'))
