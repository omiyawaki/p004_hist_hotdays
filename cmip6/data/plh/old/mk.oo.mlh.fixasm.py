...
def calc_mlh(md):
    ...
    print('\n Loading data...')
    ds=xr.open_dataset(get_mfnh('mrsos',md)) # load historical mean sm
    gpi=ds['gpi']
    sm=ds['mrsos']
    # compute mean sm change
    msmh=xr.open_dataarray(get_mfnh('mrsos',md))
    msm=xr.open_dataarray(get_mfn('mrsos',md))
    # compute hot sm change
    psmh=xr.open_dataarray(get_fnh('mrsos',md))
    psm=xr.open_dataarray(get_fn('mrsos',md))
    psmh=psmh.sel(percentile=p)
    psm=psm.sel(percentile=p)
    # average sm change
    adsm=1/2*(msm-msmh+psm-psmh)
    # future hot sm
    sm.data=sm.data+adsm
    print('\n Done.')
    ...
    with Client(n_workers=12):
        tasks=[dask.delayed(mlhmon)(mon,psm,bc) for mon in np.arange(1,13,1)]
    ...
    # save mlh
    oname=get_mfn(varn,md)
    mlh.to_netcdf(oname,format='NETCDF4')

if __name__=='__main__':
    # calc_mlh('CESM2')
    [calc_mlh(md) for md in tqdm(lmd)]
