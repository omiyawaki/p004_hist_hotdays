...
gdivt=load_mycesm('DIVT')
gdivh=load_mycesm('DIVH')
...
# RAE + turbulent flux + tfitrary heating
tfT, tfTs, _,_,_ = rae_tf(tau0s, taubl, b, beta, Fs, Fa, LH, SH, plev, ps, n)
print(tfT)
print(tfTs)

fig, ax = plt.subplots()
ax.plot(tfTs, 1e-2*ps, '.',color='tab:red')
ax.plot(gts, 1e-2*gps, '.',color='k')
ax.plot(tfT, 1e-2*plev, '-',color='tab:red',label='SCM')
ax.plot(gta, 1e-2*gplev, '-',color='k',label='CESM2')
# ax.plot(np.append(tfTs, tfT), 1e-2*np.append(ps, plev), '-',color='tab:red',label='SCM')
...
plt.tight_layout()
plt.savefig('%s/test.tf.png'%odir, format='png', dpi=600)
