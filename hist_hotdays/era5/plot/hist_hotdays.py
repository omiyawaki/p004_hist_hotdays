...
lse = ['ann','mam','son','jja'] # season (ann, djf, mam, jja, son)
...
for se in lse:
    ...
    for pc in lpc:
        ...
        vmax=1
        clf=ax.contourf(mlon, mlat, slope[str(pc)], np.arange(-vmax,vmax,0.1),extend='both', vmax=vmax, vmin=-vmax, transform=ccrs.PlateCarree(), cmap='RdBu_r')
        ax.contourf(mlon, mlat, sig[str(pc)], 3, hatches=['','....'], alpha=0, transform=ccrs.PlateCarree())
        ax.coastlines()
        ax.set_title(r'ERA5 ($1950-2020$)')
        cb=plt.colorbar(clf,location='bottom')
        cb.set_label(r'$T^{%s}_\mathrm{2\,m}$ Trend (K dec$^{-1}$)' % pc)
        plt.savefig('%s/trend_t%s.%s.pdf' % (odir,pc,se), format='pdf', dpi=300)
        plt.close()
    ...
    for pc in lpc:
        ...
        clf=ax.contourf(mlon, mlat, slope[str(pc)]/slope['50'], np.arange(0,2,0.25),extend='both', vmax=2, vmin=-0, transform=ccrs.PlateCarree(), cmap='RdBu_r')
        ...
        plt.savefig('%s/ratioT50_t%s.%s.pdf' % (odir,pc,se), format='pdf', dpi=300)
        plt.close()
