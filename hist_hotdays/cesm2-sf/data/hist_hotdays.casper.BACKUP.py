...
lfo = ['ghg'] # forcing (ghg=greenhouse gases, aaer=anthropogenic aerosols, bmb=biomass burning, ee=everything else, xaaer=all forcing except anthropogenic aerosols)
            ...
            for imem in tqdm(range(len(lmem))):
                ...
                for ln in tqdm(range(gr['lon'].size)):
                    for la in tqdm(range(gr['lat'].size)):
                        lt = t2m[:,la,ln]
                        ht2m[:,la,ln]=np.percentile(test,pc)	
                    sys.exit()
