    ...
    for fo in lfo:
        for cl in lcl:
            ...
            for imd in tqdm(range(len(lmd))):
                ...
                br=br.sel(time=br['time.year']>=byr[0])
                br=br.sel(time=br['time.year']<=byr[1])

                # select seasonal data if applicable
                if se != 'ann':
                    t2m=t2m.sel(time=t2m['time.season']==se.upper())
                    br=br.sel(time=br['time.season']==se.upper())
                ...
                cbr = np.empty([len(pc), gr['lat'].size, gr['lon'].size])
                ...
                for ln in tqdm(range(gr['lon'].size)):
                    for la in range(gr['lat'].size):
                        ...
                        lt95 = ht2m95[la,ln]
                        lt99 = ht2m99[la,ln]
                        cbr[0,la,ln]=np.nanmean(lv)
                        cbr[1,la,ln]=np.nanmean(lv[np.where(lt>lt95)])
                        cbr[2,la,ln]=np.nanmean(lv[np.where(lt>lt99)])

                pickle.dump([cbr, gr], open('%s/c%s_%g-%g.%s.pickle' % (odir,varn,byr[0],byr[1],se), 'wb'), protocol=5)	
