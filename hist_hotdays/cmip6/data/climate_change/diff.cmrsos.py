...
for fo in lfo:
    for se in lse:
        ...
        for imd in tqdm(range(len(lmd))):
            ...
            [hmrsos0, gr] = pickle.load(open('%s/cmrsos_%s.%s.pickle' % (mdir(se,'his','historical',md,varn),his,se), 'rb'))
            [hmrsos1, gr] = pickle.load(open('%s/cmrsos_%s.%s.pickle' % (mdir(se,'fut',fo,md,varn),fut,se), 'rb'))
