    ...
    for se in lse:
        for fo in lfo:
            ...
            for md in lmd:
                ...
                vlim=5
                clf=ax.contourf(mlon, mlat, diff, np.arange(-vlim,vlim,vlim/10),extend='both', vmax=vlim, vmin=-vlim, transform=ccrs.PlateCarree(), cmap='RdBu_r')
                ax.coastlines()
                ax.set_title(r'%s %s %s' % (se.upper(),md.upper(),fo.upper()))
                cb=plt.colorbar(clf,location='bottom')
                if pc==0:
                    cb.set_label(r'$\Delta \overline{SM}_\mathrm{2\,m}$ (kg m$^{-2}$)')
                else:
                    cb.set_label(r'$\Delta SM^{>%s}_\mathrm{2\,m}$ (kg m$^{-2}$)' % (pc))
