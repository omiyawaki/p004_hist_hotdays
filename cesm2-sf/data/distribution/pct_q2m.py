...
byr=[1950,2100] # output year bounds
...
for se in lse:
    for fo in lfo:
        for cl in lcl:
            if fo=='lens':
                idir='/project/mojave/cesm2/LENS/atm/day_1/QREFHT'
                odir='/project/amp/miyawaki/data/p004/hist_hotdays/cesm2-sf/%s/%s/%s' % (se,cl,fo)
            else:
                idir='/glade/campaign/cesm/collections/CESM2-SF/timeseries/atm/proc/tseries/day_1/QREFHT'
            ...
            for imem in tqdm(range(len(lmem))):
                ...
                for yr in tqdm(lyr):
                    ...
                    if fo=='lens':
                        fn = '%s/b.e21.%s.f09_g17.%s.cam.h1.QREFHT.%s.nc' % (idir,cnf,sim[imem],yr)
                    else:
                        fn = '%s/b.e21.%s.f09_g17.%s.%s.cam.h1.QREFHT.%s.nc' % (idir,cnf,sim,mem,yr)
                    ...
                    #     q2m = ds['QREFHT']
                    # else:
                    #     q2m = xr.concat((q2m,ds['QREFHT']),'time')
                    ...
                    for eyr in tqdm(np.arange(yr0,yr1+1)):
                        ...
                        pickle.dump([hq2m, gr], open('%s/hq2m_%g.%s.%s.pickle' % (odir,eyr,mem,se), 'wb'), protocol=5)	
